...
\begin{abstract}
  ...
  Within the setting of standard structures we discuss $\omega$-categoricity.
\end{abstract}
...
\begin{definition}\label{def_0}
  ...
  Finally and most relevantly, ${\EuScript L}$ contains arbitrarily many symbols for functions of sort ${\sf H}^n\to {\sf I}$.
...
\begin{notation}
  ...
  For the sake of conciseness, we write \emph{p/c} for a placeholder that should be replaced by either of \emph{p} or \emph{c.}
\end{notation}
...
Instead, working within ${\EuScript L}^{\rm p}$ comes at a price but it is convenient at it simplifies the comparision with classical model theory.
...
The requirement for $I$ to be compact seems to be a limitation, but it is not a real one.
...
\begin{fact}
  ...
  Then ${\EuScript F}$ is dense if the condition above is satisfied when  $\varphi(x)$ atomic.
\end{fact}

Using the fact above, it is immediate to verify the following.
...
The term isomorphism may sound inappropriate but it is justified by the following fact.
...
\begin{proof}
  If $a_n\in M$ is as in the fact above, then any solution of the type $p(x)\cup\big\{a_n\sim_nx\ :\ n\in\omega\big\}$ belongs to ${\rm Ccl}(M)$.
\end{proof}
...
\begin{proof}
  ...
  (2$\Rightarrow$1) \ 
  As, every c-model is atomic,  c-$\omega$-categoricity follows from Fact~\ref{fact_atomic_iso}.
\end{proof}
