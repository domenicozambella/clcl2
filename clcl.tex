...
The most straightforward examples of structures $I$ are the unit interval $[0,1]$, with the usual topology, and its homeomophic copies $\mathds{R}^{\rm p}\cup\{0,\infty\}$ and $\mathds{R}\cup\{\pm\infty\}$.
...
\begin{definition}\label{def_0}
  ...
  By \emph{model\/} we mean a two-sorted ${\EuScript L}$-structure of the form $\langle  M,I\rangle$, where $M$ ranges over all $L_{\sf H}$-structures, while $I$ is the structure we fixed above.
  ...
  Models are denoted by the domain of their home sort.
\end{definition}
...
Clearly, saturated ${\EuScript L}$-structures exist but, with the exception of trivial cases (when $I$ is finite), they are not models in the sense of Definion~\ref{def_0}.
As a remedy, below we carve out a set of formulas ${\EuScript L}^{\rm p}$, the set of positive formulas, such that every model has an positive elementary saturated extension that is also a model.
...
Note that ${\EuScript L}$ has two sorts of atomic formulas: those (equivalent to) formulas in $L_{\sf H}$ and those of the form $t(x\,;\eta)\in C$, where $C\subseteq I^n$ is compact and $t(x\,;\eta)$ is a tuple of terms of sort ${\sf H}^{|x|}\times {\sf I}^{|\eta|}\to {\sf I}$.
...
\begin{example}\label{ex_HPP}
  ...
  The relevant part of ${\EuScript L}$ contains symbols for the functions $f:{\EuScript U}^n\to{\sf I}$ that are continuous in the following sense: the inverse image of every compact $C\subseteq I$ is type-definable in $L_{\sf H}(M)$ for every $M\preceq{\EuScript U}$.
  It is easy to verify that the model $\langle{\EuScript U}, I\rangle$ is positively saturated, see definition in Section~\ref{compactness}.
\end{example}
...
To formalize the unit ball of a normed space as a model (where $I$ is the unit interval) is straightforward but pretty ugly.
...
Normed space are models with the natural interpretation of the language.
It is easy to verify that the unit ball of every model is a the unit ball of a normed space.
...
Is it possible to extend the Compactness Theorem~\ref{thm_compattezza} to a larger class of formulas?
...
\begin{definition}\label{def_dense}
  A set of formulas ${\EuScript F}\subseteq{\EuScript L}$ is \emph{dense\/} if for every positive $\varphi(x)$ and every $\varphi'>\varphi$, there is $\psi(x)\in{\EuScript F}$ such that $\varphi(x)\rightarrow\psi(x)\rightarrow\varphi'(x)$ hold in every model.

  If these implications holds in every model of $T$ we say that ${\EuScript F}$ is dense modulo $T$.
\end{definition}

By Lemma~\ref{lem_interpolation}, the set of negations of positive formulas is dense.

\begin{example}
  Write $\alpha\sim\beta$ if $\langle\alpha,\beta\rangle\in D$ for every compact neigborhood for the diagonal of ${\sf I}^2$.
  Then the set of formulas build inductively from the atomic formulas $t(x)\sim s(x)$ is a dense set.
\end{example}
...
\begin{example}
  ...
  Interestestingly this set if closed under weakening and strong negation.
\end{example}
...
%
we write \emph{$M\preceq^{\rm p} N$\/} and say that $M$ is an \emph{p-elementary submodel\/} of $N$. 

As ${\EuScript L}^{\rm p}$ are in particular $L_{\sf H}$-elementary, they are injective.
...
One convenient feature of approximate p-elementarity is that for it to hold it suffices verify (2) for any/some dense set of formulas.
...
\begin{fact}\label{fact_partial_emb}
  ...
  for every $a\in({\rm dom }f)^{|x|}$ and every ${\EuScript L}^{\rm p}$-formulas $\varphi(x)$ without quantifiers of sort ${\sf H}$.
\end{fact}
...
Our goal is to prove the Compactness Theorem for positive theories.
To bypass lengthy applications of ultrafilters, ultrapowers, and ultralimits, % -- as it would be like reinventing the wiel.
...
Our goal is to prove Lemma~\ref{lem_st} which in turn is required for the proof of the Compactness Theorem for positive theories (Theorem~\ref{thm_compattezza}).
The reader willing to accept it without proof may skip this section.

Let $\langle N, {}^*\!\!I\rangle$ be an ${\EuScript L}$-structure that extends ${\EuScript L}$-elementarily the model $M$. %$\langle M, I\rangle$.
...
\begin{proof}
  ...
  This proves the lemma for ${\EuScript L}^{\rm p}$-atomic formulas.
  Induction is immediate. 
\end{proof}
...
We say that $T$ is \emph{consistent\/} when $M\models T$  is for some model $M$.
...
\begin{theorem}
  Every model has an p-elementary extension to a positively saturated model (possibly of inaccessible cardinality).
\end{theorem}
...
\begin{proposition}\label{prop_approx}
  Let $N$ be a positively $\omega$-saturated model.
  Then 
...
A consequence of Proposition~\ref{prop_approx} is that the approximate morphisms defined in Section~\ref{morphisms} coincide with their unapproximate version when the codomain is a positively $\omega$-saturated model.
...
We denote by \emph{${\EuScript U}$\/} some large positively saturated model which we call the \emph{monster model.}
...
Below we say \emph{model\/} for p-elementary submodel of ${\EuScript U}$.
...
Hence strong models are those that realize the equivalence in Proposition~\ref{prop_approx}, e.g.\@ positively $\omega$-saturated models are strong models.
...
We define also \emph{$\mbox{p-tp}(a/A)$,} the positive type of $a$ over $A$, as the set of formulas $\big\{\varphi(x)\in{\EuScript L}^{\rm p}(A)\ :\ \varphi(a)\big\}$.
...
\end{fact}
 ...
 \begin{proof}
  (1$\Rightarrow$2) By the saturation of ${\EuScript U}$.

  
  (2$\Rightarrow$1) Suppose $\varphi(x)\notin p$.
  Then $\neg\varphi(a)$.
...
One interesting consequence of the proof of the fact above is that we can replace ${\EuScript L}^{\rm p}$ with any dense set of positive formulas closed under strong negation.
We restate the fact for an important set of positive formulas. 

Let ${\EuScript H}$ be the set of positive formulas without quantifiers of sort ${\sf I}$.
Write ${\EuScript H}\mbox{-tp}(a/A)$ for the ${\EuScript H}$-type of $a$ over $A$.
Note that ${\EuScript H}$ is a dense set closed under strong negation.
Therefore, with a similar proof as above we obtain the following.

\begin{fact}\label{fact_Hcomplete}
...
Precisely, we prove that the set formulas ${\EuScript H}$ defined above is dense, see Definion~\ref{def_dense}.

We say that partial map $f:{\EuScript U}\to{\EuScript U}$ is a ${\EuScript H}$-elementary if it preserves the truth of formulas in ${\EuScript H}$.
...
\begin{proof}
  By Fact~\ref{fact_Hcomplete}, the inverse of an ${\EuScript H}$-elementary map is also ${\EuScript H}$-elementary.
  Then we can extend the map by back-and-forth as usual.
\end{proof}

Let $A\subseteq{\EuScript U}$ be a small set throughout this section.
We apply the proposition above to obtain a strengthening of Fact~\ref{fact_Hcomplete}.

\begin{corollary}\label{corol_Lcomplete}
  Let $p(x)\subseteq{\EuScript H}$ be maximally consistent.
...
For $a,b\in {\EuScript U}^{|x|}$ we write \emph{$a\equiv_Ab$\/} if $a$ and $b$ satisfy the same ${\EuScript L}$-formulas over $A$, bearing in mind that formulas in ${\EuScript H}(A)$ or ${\EuScript L}^{\rm p}(A)$, or their approximations, suffice to test the equivalence.
...
We also need an approximation restult for the negation of positive formulas.
...
\begin{proposition}\label{prop_Tarski_Vaught}
  ...
  \begin{itemize}
    ...
    \item[3.] for every formula $\varphi(x)\in{\EuScript L}^{\rm p}(M)$
...
\begin{proof}
  ...
  \ceq{}
  {\Rightarrow}
  {\neg\varphi(a,b)}\hfill for some $b\in M$ by (3)\kern30ex
...
We say that ${\EuScript L}$ is \emph{separable\/} if there is a countable dense set ${\EuScript F}$ of positive formulas.
...
\begin{proposition}
  Assume that $I$ is second countable, and that ${\EuScript L}$ has at most countably many symbols of sort ${\sf H}^n\to {\sf I}$. 
...
Though not nedded in the sequel, it is worth mentioning that the ${\sf I}$-topology on ${\EuScript U}^{|x|}$ coincides with the product of the ${\sf I}$-topology on ${\EuScript U}$.
This can be verified by an argument similar to what used in the proof of Fact~\ref{fact_productUniformity}.
...
It is easy to see that all formulas constructed without the use of (ii) of Definion~\ref{def_LL} are ${\sf I}$-invariant.
The following proposition is an immediate consequence of compactness.
It corresponds to the Perturbation Lemma~\cite[Proposition~5.15]{HI}.
...
This definition is classical but note that, though classically every type over $M$ is finitely saisfiable in $M$, here we can only infer that $q'(x)$ is finitely satisfiable.
...
Let $I$ be $\mathds{R}^{\rm p}\cup\{0,\infty\}$, with the topology that makes it homeomorphic to the unit interval.
...
\begin{fact}
  ...
  \begin{itemize}
    \item[1.] $p(x)={\EuScript L}^{\rm p}\mbox{-tp}(a/N)$ is a Cauchy type;
...
%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%
\section{Approximate elimination of quantifiers}

Let $T$ be a positive theory.
We say that $T$ has (approximate) elimination of quantifiers if, modulo $T$ are dense modulo $T$, see Definion~\ref{def_dense}.
...
\begin{lemma}
  ...
  Then for every $\hat b\in M$ and there is a $\hat c\in N$ such that $k\cup\{\langle\hat b,\hat c\rangle\}:M\to N$ is an ${\EuScript L}^{\rm p}$-partial embedding.
\end{lemma}

\begin{proof}
  Let $\hat a$ be an enumeration of ${\rm dom} k$.
  Let $p(\hat x,\hat z)$ be the ${\EuScript L}^{\rm p}$-atomic type of $\hat b,\hat a$ in $M$.
  It suffices to show that $p(\hat x,k\hat a)$ is finitely consistent in $N$.

  We may assume that the formulas in $p(\hat x,\hat z)$ have the form $\gamma=\Pr\varphi(\hat x,\hat z)$ for some $\gamma\in I$.
  ...
  \ceq{\hfill\gamma_i}{=_\varepsilon}{\Pr\varphi_i(\hat c,k\hat a)}
  
  is true in $N$ for all $i<n$.
  Some preliminary work is required.
  For each $J\subseteq n$ define the following $L$-formulas

  \ceq{\hfill \psi_J(z)}{=}{\bigwedge_{i\in J}\exists x\;\varphi_i(x,z)}

  and
  
  \ceq{\hfill \xi_J(z)}{=}{\psi_J(z)\ \wedge\ \bigwedge_{J'\supset J}\neg\psi_{J'}(z).}

  The set of formulas $\xi_J(z)$ that are consistent define a partition of ${\EuScript U}^{|z|}$.
  For $J\subseteq n$ let $\alpha_J\in I$ be such that $p(\hat x,\hat z)$ contains the formula
  
  \ceq{2.\hfill\alpha_J}{=}{\Pr\big(\xi_J(\hat z)\big)}
  
  When $\alpha_J$ is positive, for some $\beta_{i,J}\in I$ the type $p(\hat x,\hat z)$ contains the formulas

  \ceq{3.\hfill \beta_{i,J}}{=}{\Pr\bigg(\varphi_i(\hat x,\hat z)\ \Big|\ \xi_J(\hat z)\bigg).}

  As the formulas $\varphi_i(x,z)$ define a partition, the $\beta_{i,J}$ add up to $1$ for any fixed $J$ for which they are defined (i.e.\@ such that $\alpha_J\neq0$).
  Note that $\beta_{i,J}=0$ for $i\notin J$.

  It is plain that for $i\in J$ the following is a consequence of $p(\hat x,\hat z)$

  \ceq{4.\hfill 1}{=}{\Pr\bigg(\exists x\,\varphi_i(x,\hat z)\ \Big|\ \xi_J(\hat z)\bigg)}

  Now we prove that (2) \& (4) $\Rightarrow$ (1) holds in ${\EuScript U}^\Omega$ when $\Omega$ large enough (larger than $n/\varepsilon$ suffices).
  Strictly speaking, this implication is not a positive formula, but the reader can easily verify that a suitable approximation of (2) and (4) suffices. 
  
  Let $\hat a'\in{\EuScript U}^\Omega$ satisfy (2) and (4) for every $i\in J\subseteq n$. 
  ...
  By (4), we can always find such a witness.
  
  Finally, by elementary probability theory (the theorem of total probability), we deduce

  \ceq{\hfill{\EuScript U}^\Omega}{\models}{\gamma_i=_\varepsilon\Pr\big(\varphi_i(\hat c',\hat a')\big).}

  As $k\hat a$ satisfy (2) and (4), we conclude that some $\hat c\in N$ satisfy (1).
\end{proof}
...
Below we say \emph{model\/} for ${\EuScript L}^{\rm p}$-elementariy substructure of ${\EuScript U}$.
