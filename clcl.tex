...
\begin{document}
\title{Half-standad model theory}
...
Note that ${\EuScript L}$ has two sorts of atomic formulas: those of sort ${\sf H}$ (that is, those equivalent to formulas in $L_{\sf H}$) and those of the form $t(x\,;\eta)\in C$, where $C\subseteq I^n$ is compact and $t(x\,;\eta)$ is a tuple of terms of sort ${\sf H}^{|x|}\times {\sf I}^{|\eta|}\to {\sf I}$.
...
\begin{definition}\label{def_dense}
  ...
  We say that ${\EuScript F}$ is \emph{co-dense\/} if for every $\psi(x)\in{\EuScript F}$ and every model $M$ where  $\psi(x)$ is consistent, there is a consistent positive $\varphi(x)$ and $\varphi'>\varphi$ such that $\varphi(x)\rightarrow\varphi'(x)\rightarrow\psi(x)$ holds in $M$.
\end{definition}

\begin{example}
  By Lemma~\ref{lem_interpolation}, the set of negations of positive formulas is dense and co-dense.
\end{example}

\begin{example}
  Read $\alpha=\beta$ as synonimous of $\langle\alpha,\beta\rangle\in\Delta$, where $\Delta$ is the diagonal of ${\sf I}^2$, a compact set.
  It is easy to see that the set of formulas build inductively from the atomic formulas of sort ${\sf H}$ and $t(x)=s(x)$ is dense.
\end{example}
...
The following proposition is our equivalent to the Tarski-Vaught test.

\begin{proposition}\label{prop_Tarski_Vaught}
  Let $M$ be a subset of ${\EuScript U}$.
  Let ${\EuScript F}$ be a dense set of formulas.
...
\begin{proof}
  (1$\Rightarrow$2) Assume $N\models\existsH x\,\neg\psi(x)$. Let $\varphi'>\varphi$ be such that $\varphi(x)\rightarrow\psi(x)\rightarrow\varphi'(x)$ in every ${\EuScript L}$-structure.
  Then $N\models\neg\forallH x\,\varphi(x)$ and $M\models\neg\forallH x\,\varphi(x)$ follows from (1).
  Then $M\models\neg\varphi(a)$ for some $a\in M$. Hence $M\models\varphi'(a)$ and $\varphi'(a)$ follows from (1).
  ...
  \ceq{}
  {\Rightarrow}{\existsH y\,\neg\varphi'(a,y)}\hfill for some $\varphi'>\varphi$
  
  \ceq{}
  {\Rightarrow}{\existsH y\,\neg\psi(a,y)}\hfill \rlap{for some $\psi\in{\EuScript F}$ such that $\neg\varphi'(a,y)\rightarrow\neg\psi(a,y)\rightarrow\neg\varphi(a,y)$}\kern55ex% in every ${\EuScript L}$-structure
  
  \ceq{}
  {\Rightarrow}
  {\neg\psi(a,b)}\hfill \rlap{for some $b\in M$ by (2)}\kern55ex
  ...
  \ceq{}
  {\Rightarrow}
  {M\models\neg\varphi(a,b)}\hfill by induction hypothesis
...
We say that $T$ has (approximate) elimination of quantifiers if the quantifiers free positive formulas are are dense modulo $T$, see Definion~\ref{def_dense}.
