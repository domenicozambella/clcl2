...
\begin{abstract}
  ...
  Indeed, a significant part of classical model theory withstand up to minor caveats.
  We prove that every standard structure has a \textit{positive\/} elementary extension that is standard and realizes all positive types that are finitely consistent.
  This is a positive monster model, here every formula is equivalent to an infinite, but small, disjunction of positive formulas.
  And all formulas are \textit{approximated\/} arbitrarility well by the positive ones.
  
  \noindent This approach offer a slick alternative to nonstandard analysis and real-valued model theory.
...
We rephrase some theorem and add a few remarks and examples\footnote{In the final version we may drop a few sections that do add new insight to~\cite{clcl}}.
...
% The notion of dense set of formulas (Definition~\ref{def_dense}) may be used to reduce the size of the language to a convenient size .
...
We say that a theory $T$ is \emph{${\EuScript L}$-consistent\/} if $\langle M,{}^*\!\!I\rangle\models T$ for some ${\EuScript L}$-structure $\langle M,{}^*\!\!I\rangle$.
...
%   \end{itemize}
% \end{definition}
An ${\EuScript L}$-structure $N$ is \emph{$\lambda$-p-saturated\/} if it realizes all types $p(x\,;\eta)\subseteq{\EuScript L}^{\rm p}(N)$ with fewer than $\lambda$ parameters that are finitely consistent in $N$.
...
Also the density of a set of formulas is understood modulo the theory ${\EuScript U}$.
...
We write $\Sigma^{\rm p}$ for the set of formulas of the form $\exists y\varphi(x,y)$ where $\varphi(x,y)$ is positive Boolean combination of formulas such as $\neg\psi(x,y)$ and $\psi'(x,y)$ for $\psi(x,y)$ positive and $\psi'>\psi$.
The following follows easily from the proposition above.

\begin{proposition}\label{prop_SigmaFapprox}
  Let ${\EuScript F}$ be a dense set of positive formulas.
...
A type $p(x)$ is isolated by $\varphi(x)$ if $\varphi(x)$ consistent and $\varphi(x)\rightarrow p(x)$.
We say that $p(x)$ is p-isolated over $A$ if it is isolated by $\neg\varphi(x)$ for some $\varphi(x)\in{\EuScript L}^{\rm p}(A)$.
If $p(x)$ is p-isolated over $A$ then, by Fact~\ref{prop_Fapprox}, every dense set ${\EuScript F}$ contains a consistent formula $\psi(x)$ such that $\psi'(x)$ isolates $p(x)$ for some $\psi'>\psi$.
...
Let $\langle\xi_i(x\,;z):i<\omega\rangle$ be an enumeration of a countable dense subset of ${\EuScript L}^{\rm p}_{x\,;z}(A)$.
...
\begin{itemize}
   ...
   Finally, let $\gamma_{i+1}(z)$ and $\gamma'_{i+1}>\gamma_{i+1}$ consistent positive formulas such that $\gamma'_{i+1}(z)$ implies (\#) --this exists by Proposition~\ref{prop_SigmaFapprox}.
\end{itemize}
Let $a\models\{\gamma_i(z):i<\omega\}$.
We claim that that $A,a$ does not p-isolate $p(x)$.
In fact, suppose for a contradiction $\neg\vartheta(x\,;a)\rightarrow p(x)$, where $\vartheta(x\,;z)\in{\EuScript L}^{\rm p}(A)$ and $\neg\vartheta(x\,;a)$ is consistent.
By density, and Proposition~\ref{prop_Fapprox} $\xi_i'(x\,;a)\rightarrow p(x)$ for some $i<\omega$ and some $\xi'_i>\xi_i$.
This contradicts $a\models\gamma_{i+1}(z)$.

%then $\xi_i(x\,;a)$ does not isolate $p(x)$.
Therefore the proof is complete if we can show that it is always possible to find the formula $\varphi(x)$ required above.

Suppose for a contradiction that $\neg\xi_i(x\,;z)\wedge\gamma_i(z)$ is consistent while (\#) is inconsistent for all formulas $\varphi(x)\in p$, that is, 

\ceq{\hfill\neg\xi_i(x\,;z)\wedge\gamma_i(z)}{\rightarrow}{\varphi(x).} 

This immediately implies that 

\ceq{\hfill\exists z\;\big[\neg\xi_i(x\,;z)\wedge\gamma_i(z)\big]}{\rightarrow}{p(x).}

By Fact~\ref{prop_Fapprox}, we conclude that $p(x)$ is isolated by a formula in ${\EuScript L}_{x}^{\rm p}(A)$, which contradicts our assumptions.
\end{proof}

\begin{corollary}\label{prop_OTT}
  Assume that ${\EuScript L}$ is separable and that $A$ is countable.
  Let $p(x)\subseteq{\EuScript L}(A)$ be nonisolated.
  Then there is a model $M$ containing $A$ that omits $p(x)$. 
\end{corollary}

\begin{proof}
  Appy the lemma above and the Tarski-Vaught test to obtain a countable model $M$ that does not isolate $p(x)$.
...
A theory $T$ is \emph{positively $\omega$-categorical\/} if for any two standard structures $M,N\models T$ are isomorphic.
