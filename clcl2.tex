...
\begin{fact}
  The following are equivalent
  \begin{itemize}
    \item[1.] $\varphi(x\,;z)$ is weakly stable;
    \item[2.] no $\langle a_i\,;b_i\,:\,i<\omega\rangle$ and $\tilde\varphi\perp\varphi$ satisfiy (i) and (ii$'$);
    \item[3.] no indiscernible sequence $\langle a_i\,;b_i\,:\,i<\omega\rangle$ is such that $\varphi(a_0\,;b_1)$ and $\neg\varphi(a_1\,;b_0)$.
  \end{itemize}
\end{fact}

Note that (2) says that $\varphi(x\,;z)$ is weakly stable if it is stably separated from every $\tilde\varphi\perp\varphi$, as defined in~\cite{Hr}.

Classically, (3) is equivalent to stability, but note that classical and positive monster models do not have the same indiscernible sequences. 

Let $\varphi(x\,;z)\in{\EuScript L}^{\rm p}$ and $\tilde\varphi\perp\varphi$.
A set ${\EuScript D}\subseteq{\EuScript U}^{|z|}$ is \emph{p-approximable\/} by $\varphi(x\,;z)\in{\EuScript L}^{\rm p}$ if for some $\tilde\varphi\perp\varphi$ for every finite set $B\subseteq{\EuScript U}$ there is an $a\in{\EuScript U}^{|x|}$ such that
...
We say that ${\EuScript D}$ is p-approximable \emph{from below\/} if we can strengthen (iv) by requiring that

\ceq{{\rm v.}\hfill\neg{\EuScript D}}{\subseteq}{\tilde\varphi(a\,;{\EuScript U});}

We say that ${\EuScript D}$ is \emph{externally p-definable\/} if for some formula $\tilde\varphi\perp\varphi$ the type below is finitely consistent
...
\begin{proof}
  ...
  \ceq{\hfill b_n}{\in}{{\EuScript D}\ \cap\ \bigcup^n_{i=0}\tilde\varphi(a_i\,;{\EuScript U})}.
...
\begin{proof}
  ...
  \ceq{\hfill\psi(x_0,\dots,x_m\,;z)}{=}{\tilde\varphi(x_0\,;z)\wedge\dots\wedge\tilde\varphi(x_m\,;z).}
  ...
  \ceq{\hfill b_n}{\in}{\bigcap^n_{i=0}\tilde\varphi(a_i,{\EuScript U})\smallsetminus{\EuScript D}}
