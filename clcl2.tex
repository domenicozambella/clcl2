...
\begin{notation}
  In properties that holds both for ${\EuScript L}^{\rm p}$ and ${\EuScript L}^{\rm c}$ we write \emph{${\EuScript L}^{\rm p/c}$\/} for short.
\end{notation}

The main fact to note about ${\EuScript L}^{\rm c}$ is that it is a language without equality of sort ${\sf H}$.
The difference beween ${\EuScript L}^{\rm p}$ and ${\EuScript L}^{\rm c}$ is similar to the difference between the set of absolutely integrable functions and the Lebesgue space $L^1$.
...
For $\varphi,\varphi'$ (free variables are hidden) positive formulas possibly with parameters we write \emph{$\varphi'>\varphi$\/} if $\varphi'$ is obtained by replacing in $\varphi$ each atomic formula of the form $t\in C$ with $t\in C'$, forsome $C'>C$.
...
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%
\section{Cauchy completeness}\label{Cauchy}
...
\begin{fact}\label{fact_finsat}
  The Cauchy completion of $A$ is Cauchy complete
\end{fact}

\begin{proof}
  Suppose that $a\sim'x$ is finitely satisfied in ${\rm Ccl}(A)$.
  Let $\tau,D$ be given.
  We prove that  $a\sim_{\tau,E}x$ is satisfied in $A$.
  ...
  There is some $c\in A$ such that $b\sim_{\tau,E}a$.
  Then $c\sim_{_{\tau,E}}a$ is as required.
\end{proof}
...
But first we need to define c-elementary maps which is slightly delicate due to the lack of equality.
For ease of notation we only discuss maps from ${\EuScript U}$ to itself.
...
It would be appropriate to introduce morphisms as maps ${\EuScript U}/{\sim}\rightarrow{\EuScript U}/{\sim}$ or, alternatively, as relations on ${\EuScript U}$.
Unfortunately, both these options are lead to a somewhat unpleasant notation.
The following definition is less natural but more convenient.

A map $f:{\EuScript U}\ \to\ {\EuScript U}$ is \emph{c-elementary\/} if 
\begin{itemize}
  \item[1.] $[a]\cap{\rm dom}f=\{a\}$ for every $a\in{\rm dom}f$;
  \item[2.] $\varphi(a)\rightarrow\varphi(fa)$ for every $\varphi(x)\in{\EuScript L}^{\rm c}$ and $a\in{\rm dom}f$.
\end{itemize}

We define \emph{${\EuScript L}^{\rm c}_{{\sf I}{\rm qf}}$-elementary\/} maps in a similar manner, where (2) is required to hold only for formulas in ${\EuScript L}^{\rm c}_{{\sf I}{\rm qf}}$.

A \emph{c-isomorphism\/} between two c-models $M$ and $N$ is a c-elementary map such that $|[a]\cap M|=|[fa]\cap N|=1$ for every $a\in{\rm dom}f$.

\begin{fact}
  If $f$ is a c-elementary map then $fa\not\sim fb$ for every $a,b\in{\rm dom}f$.
  Then c-elementary map are injective and their inverse is c-elementary.
\end{fact}
\begin{proof}
  As $a\not\sim b$, then $a\not\sim_{\tau,D}b$ for some $\tau,D$.
  Then there is a compact $E\subseteq I^2$, disjoint from the diagonal such that $\exists z\ \langle\tau(a,z),\tau(b,z)\rangle\in E$.
  Then $\exists z\ \langle\tau(fa,z),\tau(fb,z)\rangle\in E$ and $fa\not\sim fb$ follows.
...
\begin{theorem}\label{thm_Tarski_Vaught}
  ...
  \end{itemize}
  If, moreover, $M$ is a substructure, then (1'), (2) and (3) are also equivalent to
  \begin{itemize}
...
In this section we prove a useful lemma about isolated positive and continuous types.
The lemma yields immediately an omitting types theorem for positive types that is very close to the classical one.
However, its main applications are in the continuous version of the omitting types theorem and $\omega$-categoricity which we discuss in the sections below.
These are the notions that have interesting examples in functional analysis (here we will only discuss Hilber spaces).  
...
We say that $p(x)\subseteq{\EuScript L}^{\rm c}({\EuScript U})$ is c-isolated \emph{in the limit\/} by $A$ if for every $\tau,D$ the type $\exists y\sim_{\tau,D}x\ p(y)$ is isolated by $\neg\varphi(x)$ for some $\varphi(x)\in{\EuScript L}^{\rm c}(A)$.
% When $A$ is clear from the context (e.g., when $p(x)$ is presented as a type over $A$) we say simply p-isolated. 

\begin{fact}\label{fact_sim_n}
  Let ${\EuScript L}^{\rm c}$ be separable.
  If $p(x)\subseteq{\EuScript L}^{\rm c}(A)$ is c-isolated in the limit by $A$, then $p(x)$ is realized in ${\rm Ccl}(M)$ for every c-model $M$ containing $A$.
\end{fact}
...
\begin{proof}
  ...
  We write $x\sim_ny$ for $\sim_{\tau_n,D_n}$.
...
In words the proof above shows that if $p(x)$ is c-isolated in the limit by $M$, then there is a sequence of elements of $M$ that converges to a realization of $p(x)$. 
...
The reader can verify by herself that this is not necessary assumtion but rather a consequence of categoricity.

A c-model $M$ is c-atomic over $A$ if the types $\mbox{c-tp}(a/A)$, for all $a\in M^n$, are isolated in the limit.

\begin{fact}
  Assume that ${\EuScript L}^{\rm c}$ is separable.
  If $M$ is c-atomic over $A$ then it is c-atomic over $A,a$ for every finite tuple $a$.
\end{fact}

\begin{proof}
  Let $b\in M^n$.
  Let $p(x,y)=\mbox{c-tp}(a,b/A)$.
  Then  $p(x,a)=\mbox{c-tp}(b/A,a)$.
  Let $\neg\phi_n(x,y)$ isolate $\exists x',y'\sim_nx,y\ p(x',y')$.
  Then $\neg\phi_n(x,a)$ isolates $\exists x'\sim_nx\ p(x',a)$.
\end{proof}
