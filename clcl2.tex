...
For instance, the example below (from~\cite{clcl}) should convince the reader that ${\EuScript L}^{\rm c}$ has at least the same expressive power as real valued logic~\cite{BBHU} and~\cite{K}.% (cf.\@ the example below taken from~\cite{clcl}).%
...
We will be rather sketchy and refer the reader to~\cite{clcl} for profs and details. 
