...
\begin{example}\label{ex_HPP}
  Let ${\rm Ccl}(M)$ be a saturated structure of signature ${\EuScript L}_{\sf H}$.
  The relevant part of ${\EuScript L}$ contains symbols for the functions $f:{\rm Ccl}(M)^n\to I$ that are continuous in the following sense: the inverse image of every compact $C\subseteq I$ is type-definable in ${\EuScript L}_{\sf H}(M)$ for every $M\preceq{\rm Ccl}(M)$.
  It is easy to verify that $\langle{\rm Ccl}(M), I\rangle$ is positively saturated as defined in Section~\ref{compactness}.
\end{example}
...
\ceq{\hfill\emph{${\rm Ccl}(A)$}}{=}{\{a:a\sim'x\textrm{ is finitely satisfied in }A\}}.
...
\begin{fact}
  Let $M$ be a c-model and let $p(x)\subseteq{\EuScript L}^{\rm c}(M)$.
...
\begin{proof}
  ...
  Then also $a\sim' x$ is also finitely satisfied. 
  Hence $a\in{\rm Ccl}(M)$. 
...
\begin{proposition}\  Let $A$ be a countable set.
  ...
  Then there is a contable set $M$ containing $A$ such that ${\rm Ccl}(M)$ c-model.
  \end{itemize}
\end{proposition}
...
\begin{proof}
  ...
  Then (i) and (ii) follow from Theorem~\ref{thm_Tarski_Vaught}.
\end{proof}
...
Note that, by Theorem~\ref{thm_Tarski_Vaught}, if $A$ p-isolates $p(x)$, then $p(x)$ is realized in every p-model containing $A$.
...
% When $A$ is clear from the context (e.g., when $p(x)$ is presented as a type over $A$) we say simply p-isolated. 
Note that, by Theorem~\ref{thm_Tarski_Vaught}, if $A$ c-isolates $p(x)$, then $p(x)$ is realized in every Cauchy complete c-model containing $A$.
