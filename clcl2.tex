...
\begin{abstract}
  ...
  To demostrate this setting in action we discuss $\omega$-categoricity and stability.
\end{abstract}
...
By Theorem~\ref{thm_Tarski_Vaught}, $p(x)$ is p/c-isolated by $A$, then $p(x)$ is realized in every c-model containing $A$.
% We also have that if $p(x)$ is is realized in $M$, a p-model, then $p(x)$ is p-isolated by $M$.
% In fact, if $b\models p(x)$ then $\neg(b\neq x)$ isolates $p(x)$ where $b\neq x$ is positive by Morleyzation.
...
One possible manner to adapt this definition to our context it to replace (ii) with

\ceq{{\rm ii'.}\hfill i>j}{\Rightarrow}{\neg\varphi(a_i\,;b_j)}

where $\tilde\varphi\perp\varphi$.
We note that if require that is $n$ the same for all $\tilde\varphi$ we do not get anything new.
...
We say that $\varphi(x\,;z)\in{\EuScript L}^{\rm p}$ is \emph{approximately stable\/} if for every $\tilde\varphi\perp\varphi$ there is an $n<\omega $ such that no sequence $\langle a_i\,;b_i\,:\,i<n\rangle$ saisfies (i) and (ii') above.

\begin{fact}
  The following are equivalent
  \begin{itemize}
    \item[1.] $\varphi(x\,;z)$ is approximately stable;
    \item[2.] for every $\tilde\varphi\perp\varphi$ no sequence $\langle a_i\,;b_i\,:\,i<\omega\rangle$ saisfies (i) and (ii') above;
    \item[3.] no sequence $\langle a_i\,;b_i\,:\,i<\omega\rangle$ saisfies (i) and (ii') above for every $\tilde\varphi\perp\varphi$;
    \item[3.] There is no indiscernible sequence $\langle a_i\,;b_i\,:\,i<\omega\rangle$ such that $\varphi(a_0\,;b_1)$ and  $\tilde\varphi(a_1\,;b_0)$ for every $\tilde\varphi\perp\varphi$.
  \end{itemize}
\end{fact}
