...
\begin{abstract}
  ...
  Though little model theory is possible in general, when $I$ is a compact topological space (and the topology is coded into ${\EuScript L}$) it is possible to adapt a significant part of classical model theory to the class of standard structures.
  ...
  The main tool introduced by Henson and Iovino is the notion of approximation of a positive formulas and of its negation.\\
  \noindent
  After reviewing (and expanding) the main results of~\cite{clcl} we discuss: criteria for elimination of quantifers; omitting types; $\omega$-categoricity; stability; and indiscernibles.
\end{abstract}
...
We rephrase some theorem and add a few remarks and examples\footnote{In the final version of this paper we will omit those introductory sections that do not add new insight to~\cite{clcl}}.
...
We say that $T$ is \emph{standardly\/}  consistentif $M\models T$  is for some standard structure $M$.
...
\begin{fact}
  Any c-model $M$ is a substructure of ${\EuScript U}$.
\end{fact}

\begin{proof}
  Let $a\in M^{n}$ and let $b=fa$, where $f$ is a function symbol of sort ${\sf H}^{n}\to{\sf H}$.
  We need to prove that $b\in M$.
  It suffices to prove that $b\sim' x$ is finitely satisfied in $M$.
\end{proof}
...
\begin{proposition}\label{prop_Tarski_Vaught}
  ...
  \end{itemize}
  If $M$ is Cauchy compelte and ${\EuScript F}$ is a dense set of continuous formulas then (2) and (3) above are equivalent to
  \begin{itemize}
