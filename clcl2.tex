...
\begin{proposition}
  Fix $\tau, D$ and a finite tuple of variables $x$.
  The following are equivalent
  \begin{itemize}
    \item[1.] for every $p(x)\subseteq{\EuScript L}^{\rm c}(A)$ the type $\exists y\sim_{\tau,D}x\ p(y)$ is c-isolated;
    \item[2.] there are finitely many types $p_i(x)\subseteq{\EuScript L}^{\rm c}(A)$ such that $\exists y\sim_{\tau,D}x\ p_i(y)$ are pairwise inconsistent.
    \item[3.] there is a finite set $A\subseteq{\EuScript U}^{|x|}$ such that for every $b\in{\EuScript U}^{|x|}$ there is $a\in A$ such that $b\sim_{\tau,D}a$.
  \end{itemize}
\end{proposition}
...
\begin{proof}
  ...
  (2$\Rightarrow$3) 
  Let $A\subseteq{\EuScript U}^{|x|}$ be a set of maximal cardinality such that $a\not\sim_{\tau,D}a'$ for every distinct $a,a'\in A$.
  ...
  (2$\Rightarrow$1)
  Let $p_i(x)\subseteq{\EuScript L}^{\rm c}(A)$, for $i<n$, be such that $\exists y\sim_{\tau,D}x\ p_i(x)$ are pairwise inconsistent.
  Assume $n$ is maximal.
  Let $a_i\models p_i(x)$ and $A=\{a_0,\dots,a_{n-1}\}$..
  Let $p(x)\subseteq{\EuScript L}^{\rm c}(A)$. 
  By maximality $\exists y\sim_{\tau,D}x\ p(y)$ is consistent with some 
  By compactness there are $\varphi_i(x)\in p_i$ such that $\exists y\sim_{\tau,D}x\ \varphi_i(x)$ are pairwise inconsistent.
  By maximality $\varphi_i(x)\rightarrow\exists y\sim_{\tau,D}x\ p(y)$
