...
\begin{corollary}\label{corol_Lcomplete}
  ...
  \begin{itemize}
    ...
    \item[2.] $p(x)\leftrightarrow q(x)=\mbox{tp}(a/A)$ for some $a\in{\EuScript U}^{|x|}$.
  \end{itemize}
\end{corollary}
...
\begin{proof}
  ...
  Recall that $p(x)\leftrightarrow p'(x)$ for all $p(x)\subseteq{\EuScript L}^{\rm p}$.
  ...
  Then $q'(x)\rightarrow\varphi(x)$ and, by compactness, $\psi'(x)\rightarrow\varphi(x)$ for some $\psi'(x)\in q'$.
\end{proof}
...
We say that $T$ has (approximate) elimination of quantifiers if the quantifier-free positive formulas are dense modulo $T$, see Definion~\ref{def_dense}.
...
\begin{proof}
  ...
  When $J\in{\EuScript J}$, there are some $\beta_{i,J}\in I$ such that 

  \ceq{3.\hfill \beta_{i,J}}{=}{\Pr\bigg(\varphi_i(\hat x,\hat z)\ \Big|\ \xi_J(\hat z)\bigg).}

  are conseguence of $p(\hat x,\hat z)$.
