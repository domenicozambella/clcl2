...
\begin{definition}\label{def_LL}
  ...
  The set continuous formulas is denoted by \emph{${\EuScript L}^{\rm c}$.}
...
Normed spaces is among the of the motivating examples.
...
\begin{fact}\label{fact_st2} 
  ...
  \ceq{\hfill t^N\big(a\,;{\rm st}(\alpha)\big)}{=}{{\rm st}\big({}^*\!\tau(a\,;\alpha)\big)}
\end{fact}
\begin{proof}
...
\ceq{\hfill\emph{$x\sim_ty$}}{=}{\forallH z\ \ \tau(x,z)=\tau(y,z).}

We also define the type 

\ceq{\hfill\emph{$x\sim_{\sf I}\!y$}}{=}{\Big\{x\sim_ty\ :\ \tau(x,z)\textrm{ as above}\Big\}.}

% We define the following equivalence relation on $U$

% \ceq{\hfill\emph{$a\sim_{\sf I}\!b$}}{=}{\forallH y\ \big[\tau(a,y)=\tau(b,y)\big]}\hfill 
...
\ceq{\hfill\emph{$x\sim_{t,D}y$}}{=}{\forallH z\ \ \langle \tau(x,z),\tau(y,z)\rangle\in D}

for some compact neighborhood $D$ of $\Delta$, the diagonal of $I^2$.
Hence we define

\ceq{\hfill\emph{$x\sim'_{\sf I}\!y$}}{=}{\Big\{x\sim_{t,D}y\ :\ \tau(x,z)\textrm{ parameter-free term, } D\textrm{ compact neigborhood of }\Delta\Big\}.}

If $\tau(x,z)=t_1(x,z),\dots,t_n(x,z)$ is a tuple of terms, by $x\sim_{t,D}y$ we denote the conjunction of the formulas $x\sim_{t_i,D}y$.
...
\begin{proof}
  ...
  (3$\Rightarrow$1)  
  Let $a_{t,D}\in M$ be a realization of $a\sim_{t,D}x$ as $D$ ranges over the compact neighborhoods of the diagonal and $\tau(x,z)$ over the finite tuples parameter-free terms.
