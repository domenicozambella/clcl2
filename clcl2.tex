...
\begin{abstract}
  ...
  % Here where all formulas are \textit{approximated\/} by the positive ones.
...
A theory $T$ is \emph{positively $\omega$-categorical\/} if any two standard countable structures $M,N\models T$ are isomorphic.
The following analogue of Ryll-Nardzewski's Theorem follows from the Omitting Types Theorem as usual.
...
\begin{theorem}
  ...
  \begin{itemize}
    ...
    \item[2.] every positive type $p(x)\subseteq{\EuScript L}^{\rm p}$, for $x$ any finite tuple of variables, is p-isolated over the empty set.
  \end{itemize}
\end{theorem}
