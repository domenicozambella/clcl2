...
\begin{definition}\label{def_0}
  ...
  Finally and most relevantly, ${\EuScript L}$ contains symbols for \textit{pseudo-distal\/} functions (definition below) of sort ${\sf H}^n\times{\sf X}^m\to {\sf X}$.
...
Let $f:A\times X^m\to X$, where $A$ is a pure set.
Let $\delta$ range over the neighborhoods of the diagonal of $X^m\times X^m$.
We say that $\beta,\beta'\in X$ are \emph{pseudo-proximal\/} for $f$ if for every $\delta$ there are $a\in A$ and $\alpha,\alpha'\in X^m$ such that $(\alpha,\alpha')$, $(\beta,f(a,\alpha))$, and $(\beta',f(a,\alpha'))$ are all in $\delta$.
We say that $f$ is \emph{pseudo-distal\/} if any two pseudo-proximal points are equal.

We also define equicontinuity which is stronger condition, but it is easier to take in.
Let $\varepsilon$ and $\delta$ range over the neighborhoods of the diagonal of $X^m\times X^m$.
We say that $f$ is \emph{equicontinuous\/} if for every $\varepsilon$ there is a $\delta$ such that $\big(f(a,\alpha),f(a,\alpha')\big)\in\varepsilon$ for every $a\in A$ and every $(\alpha,\alpha')\in\delta$.
We can also say that, as $a$ ranges over $A$, the functions $f(a,\mbox{-})$ have the same modulus of uniform continuity.
It is easy to see that every equicontinuous function is pseudo-distal.
...
\begin{example}
  Let $G,X$ be an equicontinuous $G$-flow as defined in~\cite{A}.
  ...
  Then $\langle G,X\rangle$ is a standard structure.
\end{example}

% We justify the claim in the example above.
% Recall that a $G$-flow is a group $G$ that act by homeomorphisms on $X$, a compact set.
% We say that $\beta,\beta'\in X$ are proximal in the sense of~\cite{A} if for every neighborhood $\delta$ of the diagonal of $X\times X$ there is some $g\in G$ such that $\langle g\beta,g\beta'\rangle\in\delta$.
% If all proximal pairs are trivial, we say that the $G$-flow is  distal in the sense of~\cite{A}.
% It is easy to verify that the group action is distal.
% In fact if $\beta,\beta'\in X$ are 
...
\begin{lemma}\label{lem_st}
  With the notation as above.
  Let $\varphi(x\,;\eta)\in{\EuScript F}^{\rm p}$, \  $a\in N^{|x|}$, and $\alpha\in({}^*\!\!X)^{|\eta|}$ be arbitrary then $N,{}^*\!\!X\rangle\models\varphi(a\,;\alpha)$ implies that $N\models\varphi\big(a\,;{\rm st}(\alpha)\big)$
\end{lemma}
