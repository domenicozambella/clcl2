...
We denote the latter by \emph{${\EuScript L}^{\rm p/c}_{\rm at}$.}
...
\begin{fact}\label{fact_isolationpp}
  ...
  Then this can be witnessed by formulas $\psi_n(x)\in{\EuScript L}^{\rm c}(A)$ such that $\psi_n(x)\rightarrow\psi_{n+1}(x)$.
...
\begin{proof}
  ...
  \ceq{\hfill q(z)}{=}{\bigcup_{n<\omega}\Big\{\varphi(x_{\restriction n}/z_{\restriction J_0})\ \ :\ \ \varphi(x_{\restriction n})\in e,\  J_0\subseteq J, |J_0|=n\Big\}\quad\cup}

  \ceq{\hfill }{ }{\bigcup_{n<\omega}\Big\{\varphi(x_{\restriction n}/z_{\restriction J_0})\leftrightarrow\varphi(x_{\restriction n}/z_{\restriction J_1})\ \ :\ \ \varphi(x_{\restriction n})\in{\EuScript L}^{\rm p/c},\  J_i\subseteq J,\  |J_i|=n\Big\}.}
...
Now, let  $\varphi(x\,;z)\in{\EuScript L}^{\rm p}$.
We can adapt the definition above by replacing (ii) with
...
The classical notion of stabity is too strong in some circumstances.
The following definition is taken from~\cite{Hr}.
The formulas $\varphi(x\,;z),\tilde\varphi(x\,;z)\in{\EuScript L}^{\rm p}$ are \emph{stably separated\/} if there is an $n<\omega $ such that no sequence $\langle a_i\,;b_i\,:\,i<n\rangle$ saisfies (i) and (ii$'$) above.

\begin{fact}\label{fact_ssep}
  Let $\varphi(x\,;z), \tilde\varphi(x\,;z)\in{\EuScript L}^{\rm p}$.
  The following are equivalent
  \begin{itemize}
    \item[1.] $\varphi,\tilde\varphi$ are stably separated;
    \item[2.] no $\langle a_i\,;b_i\,:\,i<\omega\rangle$ satisfies (i) and (ii$'$);
    \item[3.] no indiscernible sequence $\langle a_i\,;b_i\,:\,i<\omega\rangle$ is such that $\varphi(a_0\,;b_1)$ and $\tilde\varphi(a_1\,;b_0)$.
  \end{itemize}
\end{fact}

Two simple remarks:
If $\varphi(x\,;z)$ is stable then it is stably separated from every $\tilde\varphi\perp\varphi$ and in general from any $\tilde\varphi$ that is inconsistent with $\varphi$.
A simple compactness argument shows that if $\varphi,\tilde\varphi$ is stably separated then also $\tilde\varphi,\varphi$ is stably separated.
We also need the following which is an easy consequence of (3) of Fact~\ref{fact_ssep}.

\begin{fact}
  Let $\varphi_i(x\,;z),\tilde\varphi_i(x\,;z)\in{\EuScript L}^{\rm p}$, for $i=1,\dots,n$ be stably separated.
  Then the formulas $\varphi_1\wedge\dots\wedge\varphi_n$ and $\tilde\varphi_1\vee\dots\vee\tilde\varphi_n$ are stably separated.
\end{fact}

% The following two definitions are derived from~\cite{Z} but we replace the word \textit{approximated\/} with \textit{finitely separated\/} as more appropriate.
Let $\varphi(x\,;z),\tilde\varphi(x\,;z)\in{\EuScript L}^{\rm p}$ be mutually inconsistent formulas.
Two sets ${\EuScript D},\tilde{\EuScript D}\subseteq{\EuScript U}^{|z|}$ are \emph{separated\/} by $\varphi,\tilde\varphi$ if for some $a\in{\EuScript U}^{x|}$ we have ${\EuScript D}\subseteq\varphi(a\,;{\EuScript U})$ and $\tilde{\EuScript D}\subseteq\tilde\varphi(a\,;{\EuScript U})$.
We say that they are \emph{finitely separated\/} if for every finite set $B\subseteq{\EuScript U}$ there is an $a\in{\EuScript U}^{|x|}$ such that

\ceq{{\rm iii.}\hfill B^{|z|}\, \cap\, {\EuScript D}}{\subseteq}{\varphi(a\,;{\EuScript U});}

\ceq{{\rm iv.}\hfill B^{|z|}\ \cap\ \tilde{\EuScript D}}{\subseteq}{\tilde\varphi(a\,;{\EuScript U}).}

Finally, we say finitely separated \emph{from above\/} if (iii) can be strengthened by requiring that

\ceq{{\rm v.}\hfill\varnothing}{=}{{\EuScript D}\ \cap\ \tilde\varphi(a\,;{\EuScript U}).}

Dually, we say \emph{from below\/} if (iv) can be strengthened by requiring that

\ceq{{\rm vi.}\hfill\varnothing}{=}{\tilde{\EuScript D}\ \cap\ \varphi(a\,;{\EuScript U}).}

Note that in the classical case, i.e.\@ when $\tilde{\EuScript D}=\neg{\EuScript D}$ and $\tilde\varphi\leftrightarrow\varphi$, the condition (v) and (vi) become ${\EuScript D}\subseteq\varphi(a\,;{\EuScript U})$ and $\varphi(a\,;{\EuScript U})\subseteq{\EuScript D}$ respectively.
Then we have taken over the terminology of~\cite{Z} 
only \textit{approximation\/} there becomes here \textit{finite separation\/} to avoid confusion.

\begin{remark}
  We say that ${\EuScript D},\tilde{\EuScript D}$ are \emph{externally separated\/} by $\varphi,\tilde\varphi$ if the type below is finitely consistent

  \ceq{\hfill p(x)}{=}{\big\{\varphi(x\,;b)\ :\ b\in{\EuScript D}\big\}\ \cup\ \big\{\tilde\varphi(x\,;b)\ :\ b\in\tilde{\EuScript D}\big\}.} 

  In the classical case externally separated becomes externally definable.

  It is easy to see that the following are equivalent
  \begin{itemize}
    \item[1.] ${\EuScript D},\tilde{\EuScript D}$ is finitely separated by $\varphi,\tilde\varphi$;
    \item[2.] ${\EuScript D},\tilde{\EuScript D}$ is externally separated by $\varphi,\tilde\varphi$.
  \end{itemize}
\end{remark}

The following two lemmas are required to prove a generalization of the well-known fact that sets that are externally definable by a stable formula are definable.

\begin{lemma}\label{lem_def_frombelow}
  Let $\varphi(x\,;z),\tilde\varphi(x\,;z)\in{\EuScript L}^{\rm p}$ stably separated.
  Let ${\EuScript D},\tilde{\EuScript D}$ be finitely separated from below by $\varphi,\tilde\varphi$.
  Then ${\EuScript D},\tilde{\EuScript D}$ are finitely separated separated by $\psi,\tilde\psi$ where 
  
  \ceq{\hfill\psi(z)}{=}{\varphi(x_1\,;z)\vee\dots\vee\varphi(_n\,;z)}

  \ceq{\hfill\tilde\psi(z)}{=}{\tilde\varphi(x_1\,;z)\wedge\dots\wedge\tilde\varphi(x_n\,;z)}
  
  Moreover ${\EuScript D}\cap\tilde\psi({\EuScript U})=\varnothing$.
\end{lemma}
...
\begin{proof}
  ...
  Choose $a_n\in{\EuScript U}^{|x|}$ such that $\{b_0,\dots,b_{n-1}\}\subseteq\varphi(a_n\,;{\EuScript U})$ and $\tilde{\EuScript D}\subseteq\tilde\varphi(a_n\,;{\EuScript U})$.
  This is possible by (iii) and (v).
  Now, if possible, choose $b_n$ such that

  \ceq{\hfill b_n}{\in}{{\EuScript D}\ \cap\ \bigcap^n_{i=0}\tilde\varphi(a_i\,;{\EuScript U})}.
  ...
  \ceq{1.\hfill i>j}{\Rightarrow}{\varphi(a_i\,;b_j);}

  \ceq{2.\hfill i<j}{\Rightarrow}{\tilde\varphi(a_i\,;b_j).}
  
  This shows that the pair $\tilde\varphi,\varphi$ is not stably sepatated.
  As remarked above, this contradicts the stable separation of  $\varphi,\tilde\varphi$.
\end{proof}

\begin{lemma}\label{lem_apx_frombelow}
  Let $\varphi(x\,;z),\tilde\varphi(x\,;z)\in{\EuScript L}^{\rm p}$ be stably sepatated.
  Let ${\EuScript D},\tilde{\EuScript D}$ be finitely separated by $\varphi,\tilde\varphi$.
  Then, for some $m$, the formulas 
  ...
  finitely separates ${\EuScript D}\,\tilde{\EuScript D}$ from below.
\end{lemma}

\begin{proof}
  Let $m$ be such that no sequence $\langle a_i\,;b_i : i\le m\rangle$ satisfies (i) and (ii$'$).
  Let $B\subseteq{\EuScript D}$ be finite.
  We prove that there are some $a_0,\dots,a_m$ such that $B\subseteq\varphi(a_i\,;{\EuScript U})$ for all $i\le m$ and $\tilde{\EuScript D}\cap\tilde\psi(a_0,\dots,a_m\,;{\EuScript U})=\varnothing$.
  It is easy to see that this proves (iii) and (v).
  ...
  \ceq{\hfill b_n}{\in}{\tilde{\EuScript D}\ \cap\ \bigcap^n_{i=0}\varphi(a_i,{\EuScript U})}
  ...
  In fact, otherwise we would obtain a sequence $\langle a_i\,;b_i : i\le m+1\rangle$ such that (i) and (ii$'$), contradicting our choice of $m$.
\end{proof}

\begin{theorem}
  Let $\varphi(x\,;z)\in{\EuScript L}^{\rm p}$ and $\tilde\varphi\perp\varphi$ be stably sepatated.
  Let ${\EuScript D}$ be p-approx\-imable by $\varphi,\tilde\varphi$.
  Then there are $a_{i,j}\in{\EuScript U}^{|x|}$ and $c_{i,j}\in{\EuScript U}^{|x|}$ such that

  \ceq{1.\hfill\neg{\EuScript D}}{=}{\bigcup^n_{i=0}\ \bigcap^m_{j=0}\tilde\varphi(a_{i,j},{\EuScript U})}

  \ceq{2.\hfill{\EuScript D}}{=}{\bigcup^n_{i=0}\ \bigcap^m_{j=0}\varphi(c_{i,j},{\EuScript U})}
\end{theorem}

\begin{proof}
  We prove (1). 
  The same argument applied to the pair of formulas $\tilde\varphi,\varphi$ proves (2).
  Apply Lemma~\ref{lem_apx_frombelow} to obtain $\psi(x_0,\dots,x_m\,;z)$ and  $\tilde\psi(x_0,\dots,x_m\,;z)$ that approximate ${\EuScript D}$ from below.
  Then (1) follows from Lemma~\ref{lem_def_frombelow}.
\end{proof}
