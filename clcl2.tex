...
\begin{abstract}
  ...
  We prove that their approach applies to an even larger class (which includes e.g.\@ G-flows).\\[1ex]
  \noindent
...
Let $A$ be a pure set.
We say that $f:A\times X^m\to X$ is continuous \emph{uniformly} in $A$ if for every $\varepsilon$ there is a $\delta$ such that $\big(f(a,\alpha),f(a,\alpha')\big)\in\varepsilon$ for every $a\in A$ and every $(\alpha,\alpha')\in\delta$, where $\varepsilon$ and $\delta$ range over the neighborhoods of the diagonal of $X^m\times X^m$.
...
\begin{definition}\label{def_0}
  ...
  Finally and most relevantly, ${\EuScript L}$ contains symbols for functions of sort ${\sf H}^n\times{\sf X}^m\to {\sf X}$ that are continuous uniformly in $M^n$.
...
\begin{fact}\label{fact_terms_st}
  ...
  \ceq{\hfill{}^*\!\!X}{\models}{{\rm st}\big(\tau(a,\alpha)\big)={\rm st}\big(\tau\big(a,{\rm st}(\alpha)\big)\big).}
\end{fact}
...
\begin{proof}
  ...
  We know that for every $\varepsilon$ there is a $\delta$ such that $X\models\forall x,\,(\eta,\eta')\in\delta\ \big(f(x,\eta),f(x,\eta')\big)\in\varepsilon$.
  The same formula holds in ${}^*\!\!X$.
