...
\begin{definition}\label{def_0}
  ...
  Finally and most relevantly, ${\EuScript L}$ contains symbols for functions of sort ${\sf H}^n\times{\sf X}^m\to {\sf X}$.
...
For instance, the example below should convince the reader that ${\EuScript F}^{\rm c}$ has at least the same expressive power as real valued logic, see~\cite{BBHU} and~\cite{K}.% (cf.\@ the example below taken from~\cite{clcl}).%
...
\begin{proof}
  ...
  Construct a p-elementary chain of standard structures $M_i$ and a sequence of types $p_i(x)\subseteq{\EuScript L}(M_i)$ such that $p_i(x)$ is finitely consistent in some p-elementary superstructure of $M_i$ and is realized in $M_{i+1}$.
  The positive compactness theorem ensure the existence of $M_{i+1}$.
  Let $N$ be the union of the chain.
  If the length of the chain, $\lambda$, is such that $|M_0|\le\lambda=\lambda^{<\lambda}$, we can find an enumeration of the types $p_i(x)$ such that every type over $N$ of cardinality $<\lambda$ finitely consistent in some p-elementary extension of $N$ occurs in this enumeration.
  Therefore, all these types are realized in some $M_j$ and, by p-elemenrity, $N$.
\end{proof}
...
\begin{corollary}
  In every p/c-maximal model $\{\varphi(x\,;\eta)\big\}'\leftrightarrow\varphi(x\,;\eta)$ holds for every $\varphi(x\,;\eta)\in{\EuScript F}^{\rm p}(N)$.
\end{corollary}
