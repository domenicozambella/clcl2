...
For instance, it is easy to see  that ${\EuScript L}^{\rm c}$ has at least the same expressive power as real valued logic(cf.\@ the example below taken from~\cite{clcl}).%
...
The notion of approximation is a central tool in the work of Hension and Iovino on normed spaces.
In~\cite{clcl} the natural generalization to standard structures is presented.
We recall the definition and the main properties.
...
\begin{fact}
  ...
  Then ${\EuScript F}$ is dense if the condition above is satisfied fo  $\varphi(x)$ atomic.
\end{fact}
...
\begin{example}
  ...
  In fact, first note that by the example above we can restrict to formulas built inductively from atomic formulas of the form $\tau\in[\alpha,\beta]$.
...
The following proposition of~\cite{clcl} shows that a slight amount of saturation tames the positive formulas.
...
\begin{fact}\label{fact_approx_iso}
...
\begin{proof}
  ...
  The second claim folows immediately.
\end{proof}
...
\begin{proof}
  Let $a_n\in M$ be as in the fact above.
  Then any solution of the type $p(x)\cup\big\{a_n\sim_nx\ :\ n\in\omega\big\}$ belongs to ${\rm Ccl}(M)$.  
\end{proof}
...
\begin{proof}
  ...
  Let $\psi_n(x,y)\in{\EuScript L}^{\rm c}$ be such that
