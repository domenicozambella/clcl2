...
\ceq{\hfill p_{\tau,D}(x)}{=}{\Big\{\exists y\sim_{\tau,D}x\ \varphi(y)\ \ :\ \ \varphi(x)\in p\Big\}}

By saturation 

\ceq{\hfill p_{\tau,D}(x)}{\leftrightarrow}{\exists x'\sim_{\tau,D}x\ p(x')}

But note that the equivalence may fail when evaluatec in a model that is not sufficiently saturated.
...
\begin{proof}
  Let $p(x)=q(x,a)$ for some $q(x,z)\subseteq{\EuScript L}^{\rm c}$ and $a\in A^{|z|}$.
  Let $\varphi_{\tau,D}(x)\in{\EuScript L}^{\rm c}(A)$ be such that $\neg\varphi_{\tau,D}(x)\rightarrow\exists x',y'\sim_{\tau,D} x,a\ q(x',y')$.
  Let $M$ be a c-model containing $A$.
  Let $a_{\tau,D}\in M$ be a solution of $\neg\varphi_{\tau,D}(x)$.

  Note that the following type is finitely consistent

  \ceq{\hfill q(x',y')}{\cup}{\Big\{a_{\tau,D}\sim_{\tau,D}x'\ \wedge\ b\sim_{\tau,D}y'\ \ :\ \ \tau,D\Big\}.}
  
  Let $b',a$ be a realization.
  Then $b',a\in{\rm Ccl}(M)$.
\end{proof}
...
\begin{fact}\label{fact_atomic_over_Aa}
  Let ${\EuScript L}^{\rm c}$ be separable.
  If $M$ is c-atomic over $A$ then it is c-atomic over $A,a$ for every finite tuple $a$ of elements of ${\rm Ccl}(M)$.
\end{fact}
...
\begin{proof}
  ...
  Then $\neg\varphi_n(a,y)$ is consistent and $\neg\varphi_n(a,y)\ \rightarrow\ \exists y\sim_ny'\ p(a,y')$.
\end{proof}
