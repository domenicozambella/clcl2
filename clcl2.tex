...
\begin{notation}
  ...
  For the sake of conciseness, we write \emph{p/c} as a placeholder for either one of \emph{p} or \emph{c.}
\end{notation}
...
We say that the relation $R$ is \emph{reduced\/} if there is no relation $R'\ \subset\ \mathrel{(\sim)}\circ \mathrel{R}\circ\mathrel{(\sim)}$ such that $R\ \subset\ \mathrel{(\sim)}\circ \mathrel{R'}\circ\mathrel{(\sim)}$.

\begin{fact}
  Every relation $R$ contains a reduced $R'$ such that $\mathrel{(\sim)}\circ \mathrel{R}\circ\mathrel{(\sim)}\ \ =\ \mathrel{(\sim)}\circ \mathrel{R'}\circ\mathrel{(\sim)}$.
\end{fact}
...
\begin{proof}
  (1$\Rightarrow$2) \ 
  Suppose $\langle a_i\,;b_i\,:\,i<n\rangle$ is such that (i$'$) and (ii$'$).
  As $\tilde\varphi(x\,;z)\rightarrow\neg\varphi(x\,;z)$, then $\varphi(x\,;z)$ is not $n$-stable.

  (2$\Rightarrow$1) \ 
  Suppose $\langle a_i\,;b_i\,:\,i<n\rangle$ is such that (i) and (ii).
  By Proposition~\ref{prop_Fapprox} and Lemma~\ref{lem_interpolation}, there is $\tilde\varphi\perp\varphi$ such that $\tilde\varphi(a_i\,;b_j)\rightarrow\varphi(a_i\,;b_j)$ for every $j<i<n$.
  Then $\neg$(1) follows.
\end{proof}
...
\begin{proof}
  ...
  Negate (1).
  For every $n$ let $\tilde\varphi_n\perp\varphi$ withness the failure of (2) in the fact above.
  Let $\bar x\,;\bar y=\langle x_i\,;z_i\ :\ i<\omega\rangle$.
  The type $\{\varphi(x_i\,;z_j), \tilde\varphi_n(x_j\,;z_i)\ :\ i<j<\omega\}$ is finitely consistent.
