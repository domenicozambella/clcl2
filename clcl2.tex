...
\begin{itemize}
  ...
  \item[2.] formulas of the form $t(x\,;\eta)\in C$, where $C\subseteq I^n$ is compact and $t(x\,;\eta)$ is a tuple of terms of sort ${\sf H}^{|x|}\times {\sf I}^{|\eta|}\to {\sf I}$.
\end{itemize}

Henson and Iovino have a more stringent notion of positive formulas (that they define only in the language of normed spaces).
They exclude formulas as in (1) above.
Moreover they do not consider quantifiers of sort ${\sf I}$.

Including $L_{\sf H}$ into ${\EuScript L}^{\rm p}$ simplifies the comparision with classical model theory but comes at a price.
In Section~\ref{Cauchy} we discuss this problem and introduce the set \emph{${\EuScript L}^{\rm c}$\/} of \emph{$I$-closed formulas.}
Roughly, these are formulas that define closed subsets w.r.t.\@ the topology that $I$ induces on the home sort.
...
\begin{example}[???]
  ...
  It is easy to see that the set of formulas build inductively from $L_{\sf H}$-formulas and $t(x)=s(x)$ is dense modulo ${\EuScript L}$.
\end{example}
...
We denote by \emph{${\EuScript U}$\/} some large p-saturated standard structure which we call the \emph{monster model.}
