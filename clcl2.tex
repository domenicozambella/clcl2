...
We say that a theory $T$ is \emph{${\EuScript L}$-consistent\/} if $\langle M,{}^*\!\!I\rangle\models T$ for some ${\EuScript L}$-structure $\langle M,{}^*\!\!I\rangle$ (that is, the classical notion of concistency).
We say that $T$ is \emph{st-consistent\/} if $M\models T$  is for some standard structure $M$.
...
We denote by \emph{${\EuScript U}$\/} some large p-saturated standard structure which we call the (positive) \emph{monster\/} model.
...
We denote by $T$ the theory of {${\EuScript U}$.
...
Precisely, we prove that the set formulas ${\EuScript H}$ defined above is dense (modulo $T$, the theory of ${\EuScript U}$), see Definion~\ref{def_dense}.
...
The corollary says that ${\EuScript U}$ does not distinguish between ${\EuScript L}$, ${\EuScript L}^{\rm p}$, and ${\EuScript H}$ types.
...
The following proposition is our version of the Tarski-Vaught test (which applies when the larger model is sufficiently saturated).
...
\begin{proposition}\label{prop_disjunction_dense}
  ...
  (The size of a model is the size of the domain of its home sort.)
\end{proposition}
...
\begin{theorem}[Positive Omitting Types]\label{prop_OTT}
  ...
  Then there is a model $M$ containing $A$ that omits $p(x)$. 
\end{theorem}

\begin{proof}
  As in the classical proof, appy the lemma above and the Tarski-Vaught test (Proposition~\ref{prop_Tarski_Vaught}) to obtain a countable model $M$ that does not isolate $p(x)$.
  For models, isolating and realizing are equivalent.
...
A theory $T$ is \emph{positively $\omega$-categorical\/} if any two countable models $M,N\models T$ are isomorphic.
...
As $D$ ranges over the compact neighborhoods of $\Delta$ and $\tau(x,z)$ ranges over the finite tuples parameter-free terms.
The formulas $x\sim_{\tau,D}y$ define a system of entougages on ${\EuScript U}^{|x|}$.
...
\begin{proof}
  By induction on $\varphi(x)$.
  Suppose $\varphi(x)$ as the form $\tau(x)\in C$.
  Suppose $a\models\varphi(y)$. 

\end{proof}

We say that $p(x)\subseteq{\EuScript L}^{\rm p}({\EuScript U})$ is \emph{${\sf I}$-invariant\/} if $x\sim y\rightarrow[p(x)\leftrightarrow p(y)]$.
...
We say that $p(x)$ is a \emph{Cauchy type\/} if it is consistent and $p(x)\wedge p(y)\rightarrow x\sim y$.
...
\begin{lemma}\label{lem_Cauchyequiv}
  ...
  \begin{itemize}
    ...
    \item[2.] $p(x)\leftrightarrow a\sim x$ for some/any $a\models p(x)$;
    \item[3.] $a\sim'x$ is finitely satisfiable in $M$ for some/any $a\models p(x)$.
  \end{itemize}
\end{lemma}
...
\begin{proof}
  (1$\Rightarrow$2)
  Let $a\models p(x)$.
  Substituting $a$ for $y$ in the definition of Cauchy type we obtain $p(x)\rightarrow a\sim x$
  Similarly, from the definition of ${\sf I}$-invariance we obtain $a\sim x\rightarrow p(x)$.
  ...
  Then, by the uniqueness of the limit (up to $\sim_I$-equivalence), the type $p(x)$ cointaining the formulas $x\sim_{\tau,D}a_{\tau,D}$ is as required by the lemma.
\end{proof}
...
\begin{proof}
  ...
  Then $\existsH y\sim  x\ p(y)$ is an ${\sf I}$-invariant Cauchy type that implies $\varphi$.
