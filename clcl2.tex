...
\begin{proof}
  (1$\Rightarrow$2) 
  Then $p(x)\subseteq\mbox{p-tp}(a/A)$ and, by maximality $p(x)=\mbox{c-tp}(a/A)$.
...
\ceq{\hfill\emph{$x\sim_\tau y$}}{=}{\forall z\ \ \tau(x,z)=\tau(y,z),}
...
\ceq{\hfill\emph{$x\sim_{\tau,D}y$}}{=}{\forall z\ \ \langle \tau(x,z),\tau(y,z)\rangle\in D}
...
If $\tau(x,z)=\tau_1(x,z),\dots,\tau_n(x,z)$ is a tuple of terms, by \emph{$x\sim_{\tau,D}y$\/} we denote the conjunction of the formulas $x\sim_{\tau_i,D}y$.

The formulas $x\sim_{\tau,D}y$, as $\tau$ ranges over the finite tuples parameter-free terms and $D$ over the compact neighborhoods of the diagonal, form a base for a system of entougages on ${\EuScript U}^{|x|}$.
...
\begin{fact}\label{fact_continuous}
  For every $\varphi(x)\in{\EuScript L}^{\rm p}$ that does not contain subformulas of sort ${\EuScript L}_{\sf H}$
...
Note that $a\sim x$ is a Cauchy type, therefore Cauchy  complete sets are in particular closed under $\sim$-equivalence.

We say what a type $q(x)$ is \emph{finitely satisfiable\/} in $A$ if every conjunction of formulas in $q(x)$ has a solution in $A^{|x|}$.
Literally, this definition coincide with the classical one, but in our context it is less roboust.
We may have that $p(x)$ is finitely satisfiable while $q(x)\leftrightarrow p(x)$ is not.
...
\begin{fact}\label{fact_finsat}
  Let $\bar A$ be the set of the elements that realize a Cauchy type over $A$.
  Assume that $a\sim'x$ is finitely satisfied in $A$, then $a\in\bar A$.
\end{fact}
...
We do not know in general wether $\bar A$ is Cauchy complete, nor we know if  the implication in the fact above can be reversed.
By Proposition~\ref{prop_TV_Cauchycompletion} and Remark~\ref{rem_cmodelfinsat} this is the case under extra assumptions. 
...
\begin{proof}
  ...
  \ceq{\hfill M\models\forall y\,\varphi(a,y)}{\Rightarrow}{\forall y\,\varphi(a,y)}.

  Indeed,

  \ceq{\hfill\neg\forall y\,\varphi(a,y)}
  {\Rightarrow}{\neg\forall y\,\varphi'(a,y)}\hfill \rlap{for some $\varphi'>\varphi$, by Proposition~\ref{prop_approx}}\kern52ex
  ...
  For the second part, note that the proof of (1$\Rightarrow$2) also proves (1$'\!\Rightarrow$2).
  The proof of (3$\Rightarrow$1) proves (3$\Rightarrow$1$'$) if we can show that $M$ is a substructure of ${\EuScript U}$.
...
The second part of the proposition above assumes that $M$ is Cauchy complete.
This is often a strong limitation.
Instead, we would like a property of arbitrary sets that allows to infer properies on their completions.

\begin{proposition}\label{prop_TV_Cauchycompletion}
  Let $M$ be a subset of ${\EuScript U}$.
  Let $\bar M$ be the Cauchy completion of $M$. 
  Let ${\EuScript F}$ be a c-dense set of continuous formulas.
  Assume (2) of the proposition above.
  Then $\bar M$ is Cauchy compelte c-model.
\end{proposition}

\begin{proof}
  We claim that (2) holds also for $\psi(x)\in{\EuScript F}(\bar M)$.
  ...
  By the proposition above, the proof is complete if we can show that $\bar M$ is Cauchy complete.
  Let $p(x,b)$ be a Cauchy type where $p(x,z)\subseteq{\EuScript L}^{\rm c}(M)$ and $b\in\bar M^{|z|}$.
  As $p'(x,b)\leftrightarrow a\sim'x$ for some/any $a\models p(x,b)$ it suffices to prove that $p'(x,b)$ is finitely satisfied in $M$ and apply Fact~\ref{fact_finsat}.
...
\begin{remark}
  The proof of the proposition above show that, when $M$ satisfies (2), the following are equivalent (recall that Fact~\ref{fact_finsat} proves (ii)$\Rightarrow$(i) for an arbitrary set $A$).
  \begin{itemize}
    \item[i.] $a\in\bar M$;
    \item[ii.] $a\sim'x$ is finitely satisfied in $M$.
  \end{itemize}
\end{remark}
...
\begin{proposition}\  Let $A$ be a countable set.
  \begin{itemize}
  ...
  Then the Cauchy completion of some $M$ containing $M$ is a Cauchy complete c-model.
  \end{itemize}
\end{proposition}

\begin{proof}
  As in the classical proof of the L\"owenheim-Skolem Theorem, we construct $M\subseteq{\EuScript U}$ that contains a witness of $\psi'(x)$ for every consistent $\psi(x)\in{\EuScript F}(M)$.
  Then (i) and (ii) follow from  Proposition~\ref{prop_Tarski_Vaught} and Proposition~\ref{prop_TV_Cauchycompletion} respectively.
\end{proof}
...
A type $p(x)\subseteq{\EuScript L}^{\rm c}$ is \emph{c-isolated\/} by $A$ if for every $\tau,D$ the type $\exists y\sim_{\tau,D}x\ p(y)$ is isolated by $\neg\varphi(x)$ for some $\varphi(x)\in{\EuScript L}^{\rm c}(A)$.
% When $A$ is clear from the context (e.g., when $p(x)$ is presented as a type over $A$) we say simply p-isolated. 
Note that, by Propsition~\ref{prop_Tarski_Vaught}, if $A$ c-isolates $p(x)$, then $p(x)$ is realized in every Cauchy complete c-model containing $A$.
