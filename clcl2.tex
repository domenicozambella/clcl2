...
The (partial) map $f:{\EuScript U}\rightarrow{\EuScript U}$ is \emph{p-elementary\/} if $\varphi(a)\rightarrow\varphi(fa)$ for every $\varphi(x)\in{\EuScript L}^{\rm p}$ and every $a\in({\rm dom }f)^{|x|}$.
It is easy to see that $f$ is always an injective function (because it preserves the inequality of sort ${\sf H}$).
Hence, by the following fact $f^{-1}$ is also p-elementary.

\begin{fact}\label{fact_pel_invertible}
  If $f:{\EuScript U}\rightarrow{\EuScript U}$ is a p-elementary then $\varphi(a)\leftrightarrow\varphi(fa)$ holds for every $\varphi(x)\in{\EuScript L}^{\rm p}$ and every $a\in({\rm dom }f)^{|x|}$.
\end{fact}
...
If $f:{\EuScript U}\rightarrow{\EuScript U}$ is p-elementary, ${\rm dom}f\subseteq M$, and ${\rm range}f\subseteq N$ then we say that $f:M\rightarrow N$ is p-elementary.
This is imprecise but convenient.
...
For simplicity, as we always only evaluate formulas in ${\EuScript U}$ we will safely drop the modifier \textit{approximate}.

The definition of c-elementary maps is more delicate due to the lack of equality.
...
We say that $R$ is an \emph{c-elementary relation\/}
...
By the same proof as in Fact~\ref{fact_pel_invertible} we obtain the following.
Therefore if $R$ is c-elementary, also $R^{-1}$ is c-elementary.

\begin{fact}
  If $R$ is a c-elementary relation then $\varphi(a)\leftrightarrow\varphi(b)$ for every $\varphi(x)\in{\EuScript L}^{\rm c}$ and every $\langle a,b\rangle\in R$.
\end{fact}
...
A c-elementary relation such that ${\rm dom}(R)=M$ and ${\rm range}(R)=N$ is called a \emph{c-isomorphism\/} between $M$ and $N$.

The following is noted for reference.

\begin{remark}\label{rem_c_auto}
  To be a c-automorphism of ${\EuScript U}$ the relation $R$ only needs to satisfy (\#) for formulas of the form $\tau(x)\in C$.
\end{remark}
...
Here we show that the argument can be adapted to continuous formulas.

We define ${\rm c}-{\sf I}{\rm qf}$-relations similarly to c-elementary relations but (\#) in Section~\ref{morphisms} is required to hold for formulas in ${\EuScript L}^{\rm c}_{{\sf I}{\rm qf}}$.
We ask the reader to verify that the content of that section applies verbatim to ${\EuScript L}^{\rm c}_{{\sf I}{\rm qf}}$-relation.
...
We say that the type $p(x)\subseteq{\EuScript L}^{\rm c}({\EuScript U})$ is \emph{c-isolated\/} by $A$ if for every $\varepsilon\in{\EuScript E}$ there is a formula $\varphi_\varepsilon(x)\in{\EuScript L}^{\rm c}(A)$ such that $\neg\varphi_\varepsilon(x)$ is consistent with $p(x)$ and isolates $\exists x'\sim_\varepsilon\! x\ p(x')$.

Notice that we are not simply requiring that the type $\exists x'\sim_\varepsilon\! x\ p(x')$ is isolated: we also require that $\neg\varphi_\varepsilon(x)$ is consistent with $p(x)$.
This is necessary in the proof of Lemma~\ref{lem_atomic_iso}.
...
\begin{proof}
  ...
  By the assumption above, the type 
...
In words, the proof above shows that if $p(x)$ is c-isolated, then in every c-model contains a sequence of elements that converges, in the topology induced by $I$, to a realization of $p(x)$.
...
\begin{lemma}\label{lem_kuratowskiUlam_cont}
  ...
  Let $p(x)\subseteq{\EuScript L}^{\rm c}(A)$, be non c-isolated.
...
\begin{proof}
  ...
  We go over it again to check that the stronger requiremnt of consistency does not invalidate the argument.

  Let $\varepsilon\in{\EuScript E}$ witness that $p(x)$ is not c-isolated by $A$. 
...
\begin{proof}
  By assumption $\exists y\sim_\varepsilon\! x\ p(y)$ is not c-isolated for some $\varepsilon\in{\EuScript E}$. 
  Appy Lemma~\ref{lem_kuratowskiUlam} and the Tarski-Vaught test (Theorem~\ref{thm_Tarski_Vaught}) to obtain a countable c-model $M$ that does not isolate $\exists y\sim_\varepsilon\! x\ p(y)$.
...
Let $M$ be a c-model.
We say that $M$ is \emph{atomic\/} if for every finite tuple $a$ of elements of $M$ the type $\mbox{c-tp}(a)$ is c-isolated.
...
\begin{proof}
  ...
  If $p(x)$ is not c-isolated, by the continuous omitting type theorem there is a c-model $N$ such that  ${\rm Ccl}(N)$ omits $p(x)$.
  Then ${\rm Ccl}(M)$ and ${\rm Ccl}(N)$ are not c-isomorphic, hence $T$ is not c-$\omega$-categorical.

  (2$\Rightarrow$1) As, every c-model is atomic,  c-$\omega$-categoricity follows from Fact~\ref{fact_atomic_iso} as in the classical case.
\end{proof}
...
\begin{proposition}
  ...
  \begin{itemize}
    ...
    \item[2.] for every given $\varepsilon\in{\EuScript E}$ there there are at most $n_\varepsilon<\omega$ many types $p_i(x)\subseteq{\EuScript L}^{\rm c}(A)$ such that $\exists y\sim_\varepsilon\! x\ p_i(y)$ are pairwise inconsistent.
  \end{itemize}
\end{proposition}
...
\begin{proof}
  ...
  We prove that $\exists y\sim_\varepsilon\! x\ q(y)$ is c-isolated.
