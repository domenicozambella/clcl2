...
Let $p(x)\subseteq{\EuScript L}^{\rm c}$ be a parameter-free type.
We define

\ceq{\hfill p_{\tau,D}(x)}{=}{\Big\{\exists x'\sim_{\tau,D}x\ \ \varphi(x')\ \ :\ \ \varphi(x)\in p\Big\}}

By saturation 

\ceq{\hfill p_{\tau,D}(x)}{\leftrightarrow}{\exists x'\sim_{\tau,D}x\ \ p(x')}

But note that the equivalence may fail when evaluated in a model that is not sufficiently saturated.

When $p(x)\subseteq{\EuScript L}^{\rm c}({\EuScript U})$, say $p(x)=q(x,a)$ for some $q(x,z)\subseteq{\EuScript L}^{\rm c}$ and $a\in{\EuScript U}^{|z|}$, we write $p_{\tau,D}(x)$ for $q_{\tau,D}(x,a)$.
We say that $p(x)\subseteq{\EuScript L}^{\rm c}({\EuScript U})$, is c-isolated \emph{in the limit\/} by $A$ if for every $\tau,D$ the type $p_{\tau,D}(x)$ is c-isolated by $A$.
% When $A$ is clear from the context (e.g., when $p(x)$ is presented as a type over $A$) we say simply p-isolated. 
  
\begin{fact}\label{fact_sim_n}
  % Let ${\EuScript L}^{\rm c}$ be separable.
  Let $M$ be a c-model.
  Let $p(x)\subseteq{\EuScript L}^{\rm c}(M)$ be a maximally consistent type that is c-isolated in the limit by $M$.
  Then $p(x)$ is realized in ${\rm Ccl}(M)$.
\end{fact}

\begin{proof}
  Let $p(x)=q(x,a)$ for some maximally consistent type $q(x,z)\subseteq{\EuScript L}^{\rm c}$ and $a\in M^{|z|}$.
  Pick some formulas $\varphi_{\tau,D}(x)\in{\EuScript L}^{\rm c}(M)$ such that 
  
  \ceq{\hfill\neg\varphi_{\tau,D}(x)}{\rightarrow}{\exists x',y'\sim_{\tau,D} x,a\ \ q(x',y').}

  Let $b_{\tau,D}\in M$ be a solution of $\neg\varphi_{\tau,D}(x)$.
  By maximality 
  
  \ceq{\hfill q(x',y')}{\rightarrow}{b_{\tau,D}\sim_{\tau,D}x'\ \wedge\ a\sim_{\tau,D}y'.}

  Let $b',a'\models q(x',y')$.
  Then $b'\in{\rm Ccl}(M)$ and, by Proposition~\ref{fact_continuous}, $b'\models q(x',a)$.
\end{proof}

In words, the proof above shows that if $p(x)$ is c-isolated in the limit by $M$, then there is a sequence of elements of $M$ that converges in the $I$-topology to a realization of $p(x)$.

 \begin{fact}\label{fact_wcisolatio_realization} 
   If $p(x)\subseteq{\EuScript L}^{\rm c}(M)$ is realized in ${\rm Ccl}(M)$ for some c-model $M$, then $p(x)$ is c-isolated in the limit by $M$.
 \end{fact}
 
\begin{proof}
  Let $p(x)=q(x,a)$ for some $q(x,z)\subseteq{\EuScript L}^{\rm c}$ and $a\in M^{|z|}$.
  Let $b\in{\rm Ccl}(M)^{|x|}$ realize $p(x)$.
  Let $\tau,D$ be given.
  By Fact~\ref{fact_isolation} it suffices to find a consistent formula $\varphi(x)\in{\EuScript L}^{\rm c}(M)$ and some $\varphi'>\varphi$ such that 
  
  \ceq{\hfill\varphi'(x)}{\rightarrow}{\exists x',z'\sim_{\tau,D}x,a\ \ p(x',z').}

  Let $C$ be a neighborhood of the diagonal such that $C'\circ C'\subseteq D$ for some $C'>C$.
  Let $c\in M^{|x|}$ be such that $c\sim_{\tau,C} b$.
  Then we can take $x\sim_{\tau,C}c$ as $\varphi(x)$ and $x\sim_{\tau,C'}c$ as $\varphi'(x)$.
\end{proof}
...
Let $M$ be a c-model.
Let $A\subseteq{\rm Ccl}(M)$.
We say that $M$ is \emph{c-atomic\/} over $A$ if the types $\mbox{c-tp}(a/A)$, for any finite tuple $a$ of elements of $M$, are c-isolated by $A$ in the limit.
...
\begin{fact}\label{fact_atomic_over_Aa}
  % Let ${\EuScript L}^{\rm c}$ be separable.
...
\begin{proof}
  Let $c\in M^{|z|}$.
  Let $p(x,y,z)=\mbox{c-tp}(a,b,c)$, where $a\in A^{|z|}$.
  Assume that $p(a,y,z)$ is c-isolated in the limit by $A$.
  We verify that $p(a,b,z)$ is c-isolated in the limit.
  Let $\varphi_{\tau,D}(y,z)\in{\EuScript L}^{\rm c}(A)$ be such that $\neg\varphi_{\tau,D}(y,z)$ is consistent and

  \ceq{\hfill\neg\varphi_{\tau,D}(y,z)}{\rightarrow}{\exists x',y',z'\sim_{\tau,D} a,y,z\ \ p(x',y',z').}
  ...
  \ceq{\hfill\neg\varphi_{\tau,D}(b,z)}{\rightarrow}{\exists x',y',z'\sim_{\tau,D} a,b,z\ \ p(x',y',z').}
...
\begin{proof}
  ...
  Let $b\in M$. 
  Let $p(x,y)=\mbox{c-tp}(a,b)$.
  As $M$ is atomic over $a$ by Fact~\ref{fact_atomic_over_Aa}, the type $p(a,y)$ is c-isolated in the limit.
  Then also the type $p(f_na,y)$ is c-isolated in the limit.
...
\begin{proposition}
  ...
  The following are equivalent
  \begin{itemize}
    \item[1.] every consistent $p(x)\subseteq{\EuScript L}^{\rm c}(A)$ is c-isolated in the limit;
