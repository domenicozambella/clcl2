...
\begin{abstract}
  ...
  This has been shown by Henson and Iovino for Banach spaces (see, e.g.~\cite{HI} and references cited therein) and has been generalized to a larger class of structures in~\cite{clcl}.
  We prove that their approach applies to an even larger class (which includes e.g.\@ distal G-flows).\\[1ex]
  \noindent
...
Let $A$ be a pure set.
We say that $f:A\times X^m\to X$ is \emph{equicontinuous\/} if for every $\varepsilon$ there is a $\delta$ such that $\big(f(a,\alpha),f(a,\alpha')\big)\in\varepsilon$ for every $a\in A$ and every $(\alpha,\alpha')\in\delta$, where $\varepsilon$ and $\delta$ range over the neighborhoods of the diagonal of $X^m\times X^m$.
In words we say that, as $a$ ranges over $A$, the functions $f(a,\mbox{-})$ have the same modulus of uniform continuity.

We introduce the the notion of distality.
The reader may skip this at a first reading and assume equicontinuity instead. 
We say that $\beta,\beta'\in X$ are $f$-proximal if for every $\delta$ there are $a\in A$, $\alpha,\alpha'\in X^m$ such that $(\alpha,\alpha'), (\beta,f(a,\alpha)), (\beta',f(a,\alpha'))\in\delta$.
We say that $f$ is \emph{distal\/} if any two $f$-proximal points are equal.
It is easy to see that every equicontinuous function is distal.
...
\begin{definition}\label{def_0}
  ...
  Finally and most relevantly, ${\EuScript L}$ contains symbols for distal functions that have sort ${\sf H}^n\times{\sf X}^m\to {\sf X}$.
...
They are also not interested in functions symbols of sort ${\sf H}^n\times{\sf X}^m\to {\sf X}$ with $n$ and $m$ both positive.
...
\begin{lemma}\label{fact_terms_st}
  ...
  \ceq{\hfill\langle N,{}^*\!\!X\rangle}{\models}{{\rm st}\big(\tau(a,\alpha)\big)={\rm st}\big(\tau\big(a,{\rm st}(\alpha)\big)\big).}
\end{lemma}

\begin{proof}
  We prove the fact for function symbols; the general case follows by easy induction.
  We give two proofs the first assumes equicontinuity, the second, more general, assumes only distality.
  ...
  \ceq{\hfill M}{\models}{\forall x,\eta,\eta'\big[\,(\eta,\eta')\in\delta\rightarrow \big(f(x,\eta),f(x,\eta')\big)\in\varepsilon\big].}
  ...
  Let $\beta={\rm st}(f(a,\alpha)\big)$ and $\beta'={\rm st}\big(f(a,\alpha')\big)$, where $\alpha'={\rm st}(\alpha)$.
  The following 

  \ceq{\hfill\langle N,{}^*\!\!X\rangle}{\models}{\exists x\,\exists\eta\ \Big[ (\eta,\alpha')\in\delta\ \wedge\ \big(\beta,f(x,\eta)\big)\in\delta\ \wedge\ \big(\beta',f(x,\alpha')\big)\in\delta\Big].}

  By elementarity there are $a\in M^n$ and $\alpha\in X^m$ the witness the exitentials above.
  Then $a,\alpha,\alpha'$ witness the proximality of $\beta$ and $\beta'$.
  By distality $\beta=\beta'$.
\end{proof}
...
\ceq{\hfill f^N(a,\alpha)}{=}{{\rm st}\big({}^*\!\!f(a,\alpha)\big)}\hfill for all $a\in N^n$, $\alpha\in X^{|\alpha|}$.
