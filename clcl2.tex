...
The following two examples are of a somewhat different nature.

%is inspired by~\cite{HPP}.
...
%The following example is of a somewhat different nature~\cite{LP}.

\begin{example}\label{ex_LP}
  Whil the notation of Example~\ref{ex_HPP}, let $E$ be the finest type definable (over $\varnothing$) equivalence relation on ${\EuScript U}^{|x|}$ with $<|{\EuScript U}|$ classes. 
  Let $X={\EuScript U}^{|x|}/E$.
  Let $\pi:{\EuScript U}^{|x|}\to{\EuScript U}^{|x|}/E$ be the canonical projection map.
  Then $X$ is a compact Hausdorff topological space with the topology whose closed are those $C$ such that $\pi^{-1}[C]$ is type definable possibly with parameters, see e.g.~[Lemma~3.7]\cite{LH}.
\end{example}
...
\begin{lemma}\label{lem_st}
  With the notation as above.
  For every $\varphi(x\,;\eta)\in{\EuScript F}^{\rm p}$, \  $a\in N^{|x|}$, and $\alpha\in({}^*\!\!X)^{|\eta|}$ 
