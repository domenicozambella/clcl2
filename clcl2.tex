...
Our notation and terminology diverges slightly, only where necessary.
...
We will see (cf.\@ Propositions~\ref{prop_LHapprox1} and~\ref{prop_cLHapprox1}) that the formulas in ${\EuScript L}^{\rm p/c}$ are approximated by formulas in ${\EuScript L}^{\rm p/c}$ without quantifiers of sort ${\sf I}$.
...
Let $R\subseteq M\times N$ be a binary relation.
...
We say that $R$ is an \emph{c-elementary relation\/} between $M$ and $N$ if

\ceq{\hfill M\models\varphi(a)}{\Rightarrow}{N\models\varphi(b)}\hfill  for every $\varphi(x)\in{\EuScript L}^{\rm c}$ and every $\langle a,b\rangle\in R$.
...
\ceq{\#\hfill\varphi(a)}{\leftrightarrow}{\varphi(b)}\hfill  for every $\varphi(x)\in{\EuScript L}^{\rm c}$ and every $\langle a,b\rangle\in R$.

Therefore if $R$ is c-elementary, also $R^{-1}$ is c-elementary.

In the next section we will also need the notion of ${\EuScript L}^{\rm c}_{{\sf I}{\rm qf}}$-elementary relation between ${\EuScript U}$ and itself.
This is defined similarly but (\#) above is required to hold for formulas in ${\EuScript L}^{\rm c}_{{\sf I}{\rm qf}}$.
We ask the reader to verify that the content of this section applies verbatim to ${\EuScript L}^{\rm c}_{{\sf I}{\rm qf}}$-elementary relation.

The relation $(\sim)$ is c-elementary by Fact~\ref{fact_continuous}.
Then, if the relation $R$ is c-elementary, also $\mathrel{(\sim)}\circ \mathrel{R}\circ\mathrel{(\sim)}$ is c-elementary.
...
We say that $R$ is \emph{reduced\/} if there is no $R'\ \subset\ R\ \subseteq\ \mathrel{(\sim)}\circ \mathrel{R'}\circ\mathrel{(\sim)}$.
It is easy to verify that every relation $R$ contains a reduced relation $R'$ such that 
$R\ \subseteq\ \mathrel{(\sim)}\circ \mathrel{R'}\circ\mathrel{(\sim)}$.
...
Assume $R\ =\ \mathrel{(\sim)}\circ\mathrel{R}\circ\mathrel{(\sim)}$ for simplicity.
Let $f$ be a reduced relation such that $f\ \subseteq\ R\ =\ \mathrel{(\sim)}\circ \mathrel{f}\circ\mathrel{(\sim)}$.
Then $f$ is a function defined in exacly one representative $a$ of each $(\sim)$-equivalence class $[a]\subseteq{\rm dom}(R)$.
Moreover, $f(a)$ the unique representative of $R_{/\sim}\big([a]\big)$ in the range of $f$.
...
Let $M$ and $N$ be c-models.
A c-elementary relation such that ${\rm dom}(R)=M$ and ${\rm range}(R)=N$ is called an approximate \emph{c-isomorphism\/} between $M$ and $N$.
The modifier \textit{approximate\/} is due because $R$ is c-elementary between ${\EuScript U}$ and itself.
We will only consider these c-isomorphisms, therefore we omit the modifier.
A c-automosphism a c-isomorphism of a c-model in itself.
The following is noted for future reference.

\begin{remark}\label{rem_c_auto}
  To be a c-automorphism of ${\EuScript U}$ the relation $R$ only needs to satisfy (\#) for atomic formulas of the form $\tau(x)\in C$.
\end{remark}
...
\begin{proof}
  By Fact~\ref{fact_Rcompletion} we can assume that ${\rm Ccl}(M)\subseteq{\rm dom}(R)$ and ${\rm Ccl}(N)\subseteq{\rm range}(R)$, hence a restriction of $R$ is the required c-isomorphism.
\end{proof}
...
We verify that the argument in Section~\ref{Ielimination} can be adapted to continuous formulas. 
...
\begin{proof}
  We construct by back-and-forth a sequence $f_i:{\EuScript U}\rightarrow{\EuScript U}$ of finite c-elementary reduced relations (i.e.\@ injective maps) and let 
  
  \ceq{\hfill R}{=}{\bigcup_{i\in|{\EuScript U}|}(\sim)\circ f_i\circ(\sim).}

  We will ensure that ${\EuScript U}={\rm dom}(R)={\rm range}(R)$.
  Then $R$ is a c-automorphism of ${\EuScript U}$ by Remark~\ref{rem_c_auto}.
...
If there is a countable p/c-dense set ${\EuScript F}$ of positive/continuous formulas, we say that ${\EuScript L}^{\rm p/c}$ is \emph{separable.}
...
Note that, by Theorem~\ref{thm_Tarski_Vaught}, $p(x)$ is p-isolated by $A$, then $p(x)$ is realized in every p-model containing $A$.
...
\begin{proof}
  ...
  Let $\gamma_0(z)$ be a consistent positive formulas such that $\gamma'_0(z)\rightarrow\neg\psi(z)$ for some $\gamma'_0>\gamma_0$.
  ...
  Pick $\tilde{\gamma}_i\perp\gamma_i$ such that $\gamma_i(z)\rightarrow\neg\tilde{\gamma}_i(z)\rightarrow\gamma'_i(z)$.
  
  \begin{itemize}
  \item[1.] If $\neg\xi_i(x,z)\wedge\gamma_i(z)$ is inconsistent, let $\gamma_{i+1}(z)=\gamma_i(z)$ and  $\gamma'_{i+1}(z)=\gamma'_i(z)$.
  \item[2.] Otherwise, pick $\varphi(x)\in p$ such that (\#) below is consistent
  
  (\#)\hfil $\neg\tilde{\gamma}_i(z)\ \wedge\ \exists x\,\big[\neg\xi_i(x,z)\wedge\neg\varphi(x)\big].$\kern19ex
  ...
  \ceq{\hfill\exists z\;\big[\neg\xi_i(x,z)\wedge\neg\tilde{\gamma}_i(z)\big]}{\rightarrow}{p(x).}
  
  By Fact~\ref{fact_isolation}, $p(x)$ is p-isolated, a contradiction.
\end{proof}
...
We say that the type $p(x)\subseteq{\EuScript L}^{\rm c}({\EuScript U})$ is \emph{c-isolated\/} by $A$ if for every $\tau,D$ there is a formula $\varphi(x)\in{\EuScript L}^{\rm c}(A)$ such that $\neg\varphi(x)$ is consistent with $p(x)$ and isolates $\exists x'\sim_{\tau,D}x\ p(x')$.

The following has the same proof as Fact~\ref{fact_isolation}.

\begin{fact}\label{fact_isolation}
  Let ${\EuScript F}$ be a c-dense set of continuous formulas.
  Then, for any given $\tau,D$, the following are equivalent
  \begin{itemize}
  \item[1.] there is $\varphi(x)\in{\EuScript L}^{\rm c}(A)$ such that $\neg\varphi(x)$ is consistent with $p(x)$ and isolates $\exists x'\sim_{\tau,D}x\ p(x')$;
  \item[2.] there is $\varphi(x)\in{\EuScript F}(A)$ such that $\neg\varphi(x)$ is consistent with $p(x)$ and isolates $\exists x'\sim_{\tau,D}x\ p(x')$;
  \item[3.] there is $\varphi(x)\in{\EuScript F}(A)$ consistent with $p(x)$ and $\varphi'>\varphi$ that isolates $\exists x'\sim_{\tau,D}x\ p(x')$;
  \end{itemize}
\end{fact}
...
\begin{fact}\label{fact_sim_n}
  ...
  Let $M$ be a c-model.
  Let $p(x)\subseteq{\EuScript L}^{\rm c}(M)$ be a maximally consistent type that is c-isolated by $M$.
  Then $p(x)$ is realized in ${\rm Ccl}(M)$.
\end{fact}

\begin{proof}
  For every $\tau,D$ pick a formula $\varphi_{\tau,D}(x)\in{\EuScript L}^{\rm c}(M)$ such that 
  
  \ceq{\hfill\neg\varphi_{\tau,D}(x)}{\rightarrow}{\exists x'\sim_{\tau,D} x\ \ p(x').}

  Let $b_{\tau,D}\in M$ be a solution of $\neg\varphi_{\tau,D}(x)$.
  As $p(x)$ is maximally consistent 
  
  \ceq{\hfill p(x)}{\rightarrow}{b_{\tau,D}\sim_{\tau,D}x.}

  Let $b\models p(x)$.
  Then $b\in{\rm Ccl}(M)$.
\end{proof}

In words, the proof above shows that if $p(x)$ is c-isolated by $M$, then there is a sequence of elements of $M$ that converges inthe $I$-topology to a realization of $p(x)$. 

 \begin{fact}\label{fact_wcisolatio_realization} 
   If $p(x)\subseteq{\EuScript L}^{\rm c}(A)$ is realized in ${\rm Ccl}(M)$ for some c-model $M$ containing $A$, then $p(x)$ is c-isolated by $M$.
 \end{fact}
 
\begin{proof}
  Let $b\in{\rm Ccl}(M)^{|x|}$ realize $p(x)$.
  Let $\tau,D$ be given.
  By Fact~\ref{fact_isolation} it suffices to find a continuous formula $\varphi(x)$ and some $\varphi'>\varphi$ such that $\varphi'(x)\rightarrow\exists x'\sim_{\tau,D}x\ p(x')$.
...
\begin{proof}
  ...
  (\#)\hfil $\neg\tilde{\gamma}_i(z)\ \wedge\ \exists x\,\big[\neg\xi_i(x,z)\wedge\neg\exists x'\sim_{\tau,D}x\ \varphi(x')\big].$\kern15ex
  ...
  Let $a\models\{\gamma_i(z):i<\omega\}$.
  We claim that that $A,a$ does not c-isolate $p(x)$, witnessed by $\tau,D$.
  Otherwise $\neg\xi_i(x,a)\rightarrow\exists x'\sim_{\tau,D}x\  p(x')$ for some $\neg\xi_i(x,a)$ consistent with $p(x)$.
  This contradicts $a\models\gamma_{i+1}(z)$.
  ...
  \ceq{\hfill\exists z\;\big[\neg\xi_i(x,z)\wedge\neg\tilde{\gamma}_i(z)\big]}{\rightarrow}{\exists x'\sim_{\tau,D}x\ p(x').}
...
\begin{theorem}[Continuous Omitting Types]\label{thm_cOTT}
  Let ${\EuScript L}^{\rm c}$ be separable.
  Let $A$ be countable.
  Assume also that $p(x)\subseteq{\EuScript L}^{\rm c}(A)$ is not c-isolated.
...
Let $M$ be a c-model.
We say that $M$ is \emph{c-atomic\/} over $A$ if for every finite tuple $a$ of elements of $M$ the type $\mbox{c-tp}(a/A)$ is c-isolated by $A$.

% When ${\EuScript L}^{\rm c}$ is separable, there is a sequence $\langle\tau_n,D_n:n<\omega\rangle$ such that for every $\tau,D$ there is an $n<\omega$ such that $x\sim_{\tau_n,D_n}y\ \rightarrow\ x\sim_{\tau,D}y$. 
% We write \emph{$x\sim_ny$\/} for $\sim_{\tau_n,D_n}$.

\begin{fact}[False]\label{fact_atomic_over_Aa}
  % Let ${\EuScript L}^{\rm c}$ be separable.
  Let $M$ be a c-model that is c-atomic over $A$.
  Then $M$ is c-atomic over $A,b$ for every finite tuple $b$ of elements of ${\rm Ccl}(M)$.
\end{fact}

\begin{proof}
  Let $a,b\in M^{|x,y|}$.
  Let $p(x,y)=\mbox{c-tp}(a,b)$.
  Assume that $p(x,y)$ is c-isolated by $A$.
  We verify that $p(x,b)$ is c-isolated.
  Let $\varphi_{\tau,D}(x,y)\in{\EuScript L}^{\rm c}(A)$ be such that $\neg\varphi_{\tau,D}(x,y)$ is consistent with $p(x,y)$ and 

  \ceq{\hfill\neg\varphi_{\tau,D}(x,y)}{\rightarrow}{\exists x',y',\sim_{\tau,D} x,y\ \ p(x',y').}

  % Note that $\neg\varphi_{\tau,D}(b,c)$ holds for all $\tau,D$.
  %  $\neg\varphi_{tau,D}(x,c)$ is consistent
  % Then $\neg\varphi_n(b,z)\ \rightarrow\ \exists z',x',y'\sim_{\tau,D} a,x,c\ p(z',x',y')$.
  Then, clearly

  \ceq{\hfill\neg\varphi_{\tau,D}(x,b)}{\rightarrow}{\exists x',y'\sim_{\tau,D} x,b\ \ p(x',y').}

  Then we are only left to prove that $\neg\varphi_{\tau,D}(b,z)$ is consistent.
\end{proof}

\begin{fact}\label{fact_atomic_iso}
  Let $M$ and $N$ be countable c-atomic c-models.
...
\begin{proof}
  ...
  Let $b\in M$. 
  Let $p(x,y)=\mbox{c-tp}(a,b)$.
  As $M$ is atomic over $a$ by Fact~\ref{fact_atomic_over_Aa}, the type $p(a,y)$ is c-isolated.
  Then also the type $p(f_na,y)$ is c-isolated.
  Let $c\in{\rm Ccl}(N)$ realize $p(f_na,y)$.
...
\begin{theorem}
  ...
  \begin{itemize}
    ...
    \item[2.] every consistent type $p(x)\subseteq{\EuScript L}^{\rm c}$, for $x$ any finite tuple of variables, is c-isolated.
  \end{itemize}
\end{theorem}

\begin{proof}
  (1$\Rightarrow$2) 
  This is just the classical argument.
  If $p(x)$ is consistent it is realized in some countable c-model $M$.
  If $p(x)$ is not c-isolated, by the continuous omitting type theorem there is a c-model $M$ such that  ${\rm Ccl}(N)$ omits $p(x)$.
  Then ${\rm Ccl}(M)$ and ${\rm Ccl}(N)$ are not c-isomorphic, hence $T$ is not c-$\omega$-categorical.

  (2$\Rightarrow$1) As, every model is c-atomic,  c-$\omega$-categoricity follows from Fact~\ref{fact_atomic_iso} as in the classical case.
\end{proof}
...
\begin{proposition}
  ...
  The following are equivalent
  \begin{itemize}
    \item[1.] every consistent $p(x)\subseteq{\EuScript L}^{\rm c}(A)$ is c-isolated;
    \item[2.] for every given $\tau, D$ there there are at most $n_{\tau, D}<\omega$ many types $p_i(x)\subseteq{\EuScript L}^{\rm c}(A)$ such that $\exists y\sim_{\tau,D}x\ p_i(y)$ are pairwise inconsistent.
  \end{itemize}
\end{proposition}
...
\begin{proof}
  (1$\Rightarrow$2) 
  Let $\tau, D$ be given.
  Let $p_i(x)\subseteq{\EuScript L}^{\rm c}$, for $i<\lambda$, be such that $\exists y\sim_{\tau,D}x\ p_i(y)$ are pairwise inconsistent.
  Suppose that $\lambda$ is maximal and, for a contradiction, that it is infinite.
  Let $\neg\varphi_i(x)$, where $\varphi_i(x)\in{\EuScript L}^{\rm c}$, be the formulas that isolate $\exists y\sim_{\tau,D}x\ p_i(x)$.
  As these types are pairwise inconsistent, then the continuous type $q(x)=\{\varphi_i(x):i<\lambda\}$ is consistent.
  The type $\exists y\sim_{\tau,D}x\ q(x)$ is inconsistent with all $\exists y\sim_{\tau,D}x\ p_i(x)$.
  This contradicts the maximality of $\lambda$.

  (2$\Rightarrow$1?) 
  Let $\tau, D$ be given and pick a compact neighborhood of the diagonal $E$ such that $E\circ E\circ E\subseteq D$.
  Let $p_i(x)\subseteq{\EuScript L}^{\rm c}(A)$, for $i<n$, be such that $\exists y\sim_{\tau,E}x\ p_i(x)$ are pairwise inconsistent.
  ...
  In fact, if $q(x)=\mbox{c-tp}(a/A)$, where $a$ does not satisfy the formula above, then $\exists y\sim_{\tau,E}x\ q(y)$ would be inconsistent with every $\exists y\sim_{\tau,E}x\ p_i(x)$.
  This contradicts the maximality of $n$.

  Let $q(x)\subseteq{\EuScript L}^{\rm c}(A)$.
  We prove that $\exists y\sim_{\tau,D}x\ q(y)$ is c-isolated.
  By maximality $\exists y\sim_{\tau,E}x\ q(y)$ is consistent with, say, $\exists y\sim_{\tau,E}x\ \varphi_1(x)$.
  Without loss of generality, assume $q(x)\rightarrow\exists y\sim_{\tau,E}x\ \varphi_1(x)$.
  Note that
  ...
  \ceq{1.\hfill\neg\bigvee_{i=1}^n\varphi_i(x)}{\rightarrow}{\exists y\sim_{\tau,D}x\ q(y)}
