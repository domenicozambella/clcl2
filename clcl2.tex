...
\begin{abstract}
  ...
  When $I$ is a compact topological space (and ${\EuScript L}$ meets a few requirements) it is possible to adapt a significant part of model theory to the restricted class standard strutures.
  ...
  The main tool is the notion of approximations of positive formulas and of their negation.
  ...
  Finally, we discuss criteria for elimination of quantifers, omitting types, $\omega$-categoricity, stability, and indiscernibles within the setting of standrd structures.
\end{abstract}
...
\begin{example}
  Let $I=[0,1]$.
  The set of formulas build inductively from atomic formulas of the form $\tau\in\{0\}$, is c-dense modulo ${\EuScript L}$.
...
\begin{example}
  In Section~\ref{Ielimination} we will prove that the set of positive/continuous formulas without quantifiers of sort ${\sf I}$ is p-dense/c-dense.
\end{example}
...
We write \emph{$\Sigma^{\rm p}$\/} for the set of formulas of the form $\exists y\,\varphi(x,y)$ where $\varphi(x,y)$ is a positive Boolean combination of formulas such as $\neg\psi(x,y)$ and $\psi'(x,y)$ for $\psi(x,y)$ positive and $\psi'>\psi$.
