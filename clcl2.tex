...
\begin{definition}\label{def_LL}
  ...
  We write \emph{${\EuScript L}^{\rm p/c}$\/} when we refer simultaneously to both ${\EuScript L}^{\rm p}$ and ${\EuScript L}^{\rm c}$. 
...
We will see (cf.\@ Propositions~\ref{prop_LHapprox1}) that the formulas in ${\EuScript L}^{\rm p/c}$ are approximated by formulas without quantifiers of sort ${\sf I}$.
...
\begin{proof}
  (1$\Rightarrow$2) 
  Then $p(x)\subseteq\mbox{p-tp}(a/A)$ and, by maximality $p(x)=\mbox{p-tp}(a/A)$.
...
The class ${\EuScript L}^{\rm c}_{{\sf I}{\rm qf}}$ and the \emph{${\EuScript L}^{\rm c}_{{\sf I}{\rm qf}}$-elementary\/} maps are defined in analogy to their positive variants.
...
\begin{theorem}\label{thm_Tarski_Vaught}
  ...
  \end{itemize}
  If, moreover, $M$ is a substructure, then (2) and (3) above are equivalent to
  \begin{itemize}
...
\begin{remark}\label{rem_Tarski_Vaught}
  The theorem above shows in particular that or every substructure $M$ the following are equivalent
  \begin{itemize}
    \item[1.] $M$ is a c-model;
...
We say that $p(x)\subseteq{\EuScript L}^{\rm c}({\EuScript U})$ is \emph{c-isolated\/} by $A$ if for every $\tau,D$ the type $\exists y\sim_{\tau,D}x\ p(y)$ is isolated by $\neg\varphi(x)$ for some $\varphi(x)\in{\EuScript L}^{\rm c}(A)$.
% When $A$ is clear from the context (e.g., when $p(x)$ is presented as a type over $A$) we say simply p-isolated. 

\begin{fact}  
  Let ${\EuScript L}^{\rm c}$ be separable.
  If $p(x)\subseteq{\EuScript L}^{\rm c}(A)$ is c-isolated by $A$, then $p(x)$ is realized in every Cauchy complete c-model containing $A$.
\end{fact}

\begin{proof}
  As ${\EuScript L}^{\rm c}$ is separable, there is a sequence $\langle\tau_n,D_n:n<\omega\rangle$ such that 
  
  \ceq{\hfill x\sim y}{\leftrightarrow}{\bigwedge\big\{x\sim_{\tau_n,D_n}y\ :\ n<\omega\big\}.}
  ...
  Then $b\sim'x$ is finitely satisfied in $M$ and therefore $b\in M$.
\end{proof}
