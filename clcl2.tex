...
\ceq{{\rm i.}\hfill i<j}{\Rightarrow}{\varphi(a_i\,;b_j)\ \wedge\ \neg\varphi(a_j\,;b_i)}\hfill  for every $i,j<n$.

We say that $\varphi(x\,;z)$ is \emph{$\varepsilon$-stable\/} if for some $n<\omega$ there is no sequence $\langle a_i\,;b_i\,:\,i<n\rangle$ such that 

\ceq{{\rm i'.}\hfill i<j}{\Rightarrow}{\varphi(a_i\,;b_j)\ \wedge\ \neg\varphi_\varepsilon(a_j\,;b_i)}\hfill  for every $i,j<n$.

where $\varphi_\varepsilon(a_j\,;b_i)$ stands for $\exists x,z\sim_\varepsilon a_j,b_i\ \varphi(x\,;z)$.
We say that $\varphi(x\,;z)$ is \emph{$\sim$-stable\/} if it is $\varepsilon$-stable for every $\varepsilon$.

The set ${\EuScript D}\subseteq{\EuScript U}^{|z|}$ is \emph{$\varepsilon$-approximated\/} by $\varphi(x\,;z)$ if for every finite set $B\subseteq{\EuScript U}^{|z|}$ there is an $a\in{\EuScript U}^{|x|}$ such that for every $b\in B$

\ceq{{\rm ii.}\hfill B\ \cap\ {\EuScript D}}{\subseteq}{B\ \cap\ \varphi(a\,;{\EuScript U});}

\ceq{{\rm iii.}\hfill B\ \cap\ \neg{\EuScript D}_\varepsilon}{\subseteq}{B\ \cap\ \neg\varphi_\varepsilon(a\,;{\EuScript U}).}

Finally, we say $\varepsilon$-approximated \emph{from above\/} if we also have

\ceq{{\rm iv.}\hfill{\EuScript D}}{\subseteq}{\varphi_\varepsilon(a\,;{\EuScript U}).}


\begin{lemma}
  Let $\varphi(x\,;z)$ be $\varepsilon$-stable.
  If ${\EuScript D}$ is $\varepsilon$-approximable by $\varphi(x\,;z)$ then ${\EuScript D}$ is $\varepsilon$-approx\-imable from above by $\varphi(x_0\,;z)\vee\dots\vee\varphi(x_m\,;z)$, for some $m$.
\end{lemma}
...
\begin{proof}
  ...
  \ceq{\hfill B\cap\neg{\EuScript D}_\varepsilon}{\subseteq}{\neg\varphi_\varepsilon(a_i\,;{\EuScript U})}\hfill for every $0\le i\le m$;

  \ceq{\hfill{\EuScript D}}{\subseteq}{\bigcup_{i=0}^m\varphi_\varepsilon(a_i,\,;{\EuScript U}).}
  ...
  \ceq{\hfill B\cap\neg{\EuScript D}_\varepsilon}{\subseteq}{\neg\varphi_\varepsilon(a_n\,;{\EuScript U}).}

  This is possible by (ii) and (iii).
  Now, if possible, choose $b_n$ such that

  \ceq{\hfill b_n}{\in}{{\EuScript D}\smallsetminus\bigcup_{i=0}^n\varphi_\varepsilon(a_i,\,;{\EuScript U})}.
  ...
  In fact, otherwise we would contradict the choice of $m$ as the sequence $\langle a_i\,;b_i : i\le m\rangle$, with the reversed order, satisfies (i$'$).
\end{proof}

\begin{lemma}\label{lem_def_frombelow}
  Let $\varphi(x\,;z)$ be $\varepsilon$-stable.
  Let ${\EuScript D}$ be $\varepsilon$-approximable by $\varphi(x\,;z)$ from above.
  Then for some $m$ and some $a_i\in{\EuScript U}^{|x|}$

  \ceq{\hfill\bigcap^n_{i=0}\varphi(a_i\,;{\EuScript U})}{\subseteq}{{\EuScript D}_\varepsilon}.
\end{lemma}

\begin{proof}
  The elements $a_n$ are defined recursively together with some auxiliary elements $b_n$.
  Suppose $b_0,\dots,b_{n-1}\notin{\EuScript D}_\epsilon$ have been defined.
  We first define $a_n$, then $b_n$.
  Choose $a_n\in{\EuScript U}^{|x|}$ such that $\{b_0,\dots,b_{n-1}\}\subseteq\neg\varphi_\varepsilon(a_n\,;{\EuScript U})$ and ${\EuScript D}\subseteq\varphi_\varepsilon(a_n\,;{\EuScript U})$.
  This is possible by (iii) and (iv).
  Now, if possible, choose $b_n$ such that

  \ceq{\hfill b_n}{\in}{\bigcap^n_{i=0}\varphi(a_i\,;{\EuScript U})\ \smallsetminus\ {\EuScript D}_\epsilon}.

  If this is not possible, then $a_0,\dots,a_n$ are those required by the lemma.
  We claim that at some $n$ the procedure halts.
  In fact, otherwise we would contradict the choice of $m$ as we we would obtain a sequence $\langle a_i\,;b_i : i\le m\rangle$ such that (i$'$).
\end{proof}

\begin{lemma}\label{lem_apx_frombelow}
  Let $\varphi(x\,;z)\in{\EuScript L}$ be $\varepsilon$-stable.
  Let ${\EuScript D}$ be $\varepsilon$-approximable by $\varphi(x\,;z)$.
  Then the formula 

  \ceq{\hfill\psi(x_0,x_1\,;z)}{=}{\varphi(x_0\,;z)\vee\varphi(x_1\,;z)}

  is also  $\varepsilon$-stable.
\end{lemma}

\begin{proof}
  By the usual finite Ramsey's argument.
\end{proof}

\begin{theorem}
  Let $\varphi(x\,;z)\in{\EuScript L}$ be $\varepsilon$-stable.
  Let ${\EuScript D}$ be $\varepsilon$-approximable by $\varphi(x\,;z)$.
  Then for some $m,n$ and some $a_{i,j}\in{\EuScript U}^{|x|}$

  \ceq{\hfill\bigcap^n_{i=0}\ \bigcup^m_{j=0}\varphi(a_{i,j}\,;{\EuScript U})}{\subseteq}{{\EuScript D}_\epsilon}.


  \ceq{\hfill{\EuScript D}}{\subseteq}{\bigcap^n_{i=0}\ \bigcup^m_{j=0}\varphi_\epsilon(a_{i,j}\,;{\EuScript U})}.
