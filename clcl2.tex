...
\begin{abstract}
  ...
  A \textit{standard\/} structure is an ${\EuScript L}$-structure of the form $\langle M,X\rangle$.
...
\begin{definition}\label{def_0}
  Let ${\EuScript L}$ be a two sorted language that expands both ${\EuScript L}_{\sf H}$ and ${\EuScript L}_{\sf X}$.

  A \emph{standard structure\/} is a two-sorted ${\EuScript L}$-structure of the form $\langle M,X\rangle$, where $M$ is any ${\EuScript L}_{\sf H}$-structure.
  We write \emph{${\sf H}$} and \emph{${\sf X}$\/} to denote the two sorts of ${\EuScript L}$.
  ...
  Finally and most relevantly, ${\EuScript L}$ contains symbols for functions of sort ${\sf H}^n\to {\sf X}$.
...
% Models have the form $\langle M _1\dots,M_k, X\rangle$.
% The new symbols in the language ${\EuScript L}$ are of sort ${\sf H}_1^{n_1}\dots,{\sf H}_k^{n_k}\to {\sf X}$.

Clearly, saturated ${\EuScript L}$-structures exist but, with the exception of trivial cases (when $X$ is finite), these are not standard.
...
As usual, ${\EuScript L}_{\sf X}$, ${\EuScript L}_{\sf H}$, and ${\EuScript L}$ denote both first-order languages and the corresponding set of formulas.
...
% The symbols ${\EuScript F}^{\rm p}$ and ${\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}$ defined below denote only sets of ${\EuScript L}$-formulas.
...
\begin{itemize}
  ...
  \item[2.] $\tau(x\,;\eta)\in C$, where $C\subseteq X^n$ is a compact set and $\tau(x\,;\eta)$ is a tuple of terms of sort ${\sf H}^{|x|}\times {\sf X}^{|\eta|}\to {\sf X}$ (up to equivalence, equality has also this form).
\end{itemize}

\begin{definition}\label{def_LL}
  A formula in ${\EuScript L}$ is \emph{positive\/} if it uses only the Boolean connectives $\wedge$, $\vee$; the quantifiers $\forall\raisebox{1.1ex}{\scaleto{\sf H}{.8ex}\kern-.2ex}$, $\exists\raisebox{1.1ex}{\scaleto{\sf H}{.8ex}\kern-.2ex}$ of sort ${\sf H}$; and the quantifiers $\forall\raisebox{1.1ex}{\scaleto{\sf X}{.8ex}\kern-.2ex}$, $\exists\raisebox{1.1ex}{\scaleto{\sf X}{.8ex}\kern-.2ex}$ of sort ${\sf X}$.
  ...
  We will use Latin letters $x,y,z$ for variables of sort ${\sf H}$ and Greek letters $\eta,\varepsilon$ for variables of sort ${\sf X}$.
...
The positive formulas of Henson and Iovino correspond to those formulas in ${\EuScript F}^{\rm c}$ that do not have quantifiers of sort ${\sf X}$.

Allowing quantifiers of sort ${\sf X}$ comes almost for free.
In Section~\ref{cIelimination} will see that the formulas in ${\EuScript F}^{\rm c}$ are approximated by formulas in ${\EuScript F}^{\rm c}$ without quantifiers of sort ${\sf X}$.
Instead, working within ${\EuScript F}^{\rm p}$ comes at a price but it is convenient as it simplifies the comparision with classical model theory.
...
For instance, the example below (from~\cite{clcl}) should convince the reader that ${\EuScript F}^{\rm c}$ has at least the same expressive power as real valued logic, see~\cite{BBHU} and~\cite{K}.% (cf.\@ the example below taken from~\cite{clcl}).%
...
\begin{example}\label{ex_HPP}
  ...
  The relevant part of ${\EuScript L}$ contains symbols for the functions $f:{\EuScript U}^n\to X$ that are continuous in the following sense: the inverse image of every compact $C\subseteq X$ is type-definable in ${\EuScript L}_{\sf H}(M)$ for every $M\preceq{\EuScript U}$.
  %It is easy to verify that $\langle{\EuScript U}, X\rangle$ is positively saturated as defined in Section~\ref{compactness}.
\end{example}
...
E.g.\@ can we allow in ${\EuScript L}$ (and in ${\EuScript F}^{\rm p/c}$) function symbols of sort ${\sf H}^n\times {\sf X}^m\to {\sf X}$ with both $n$ and $m$ positive?
These would have natural interpretations, e.g.\@ a group acting on a compact set $X$.
...
For $C,C'$ compact substes of $X^n$, we write $C'>C$ if $C'$ is a neighborhood of $C$.
...
% The approximate ${\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}$-morphisms we define below are the maps that preserve Henson-Iovino approximate satisfaction.
...
We will be sketchy and refer the reader to~\cite{clcl} for proofs and details. 
...
Let $\langle N, {}^*\!\!X\rangle$ be an ${\EuScript L}$-structure that extends ${\EuScript L}$-elementarily the standard structure $M$. %$\langle M, X\rangle$.
...
% The set of the realizations of ${\rm m}_\beta(\eta)$ in ${}^*\!\!X$ is known to nonstandard analysts as the monad of $\beta$. 
...
\ceq{\hfill{}^*\!\!X}{\models}{\alpha\in C\ \rightarrow\ {\rm st}(\alpha)\in C.}
\end{fact}

% \begin{proof}
%   Assume ${\rm st}(\alpha)\notin C$.
%   By normality there is a compact set $D$ disjoint from $C$ that is a neighborhood of ${\rm st}(\alpha)$.
%   Then  ${}^*\!\!X\models\alpha\in D\subseteq\neg C$.
% \end{proof}

We need also the following.

\begin{fact}\label{fact_terms_st}
  For every $\alpha\in({}^*\!\!X)^{|\alpha|}$ and every function symbol $f$ of sort ${\sf X}^{|\alpha|}\to{\sf X}$

  \ceq{\hfill{}^*\!\!X}{\models}{{\rm st}\big(f(\alpha)\big)=f\big({\rm st}(\alpha)\big).}
\end{fact}

\begin{proof}
  By the definition of st(-) it suffices to prove that ${}^*\!\!X\models f(\alpha)\in D$ for every compact neighborhood $D$ of $f\big({\rm st}(\alpha)\big)$.
  
  Fix one such $D$.
  Then ${\rm st}(\alpha)\in f^{-1}[D]$ .
  By continuity $f^{-1}[D]$ is a compact neighborhood of ${\rm st}(\alpha)$.
  Therefore ${}^*\!\!X\models \alpha\in f^{-1}[D]$ and, as $X\preceq{}^*\!\!X$ we obtain ${}^*\!\!X\models f(\alpha)\in D$.
\end{proof}

The \emph{standard part of $\langle N,{}^*\!\!X\rangle$\/} is the standard structure $\langle N,X\rangle$ that interprets the symbols $f$ of sort ${\sf H}^n\to {\sf X}$ as the functions
...
\begin{fact}\label{fact_st2} 
  ...
  Let $\tau(x\,;\eta)$ be a term of sort ${\sf H}^{|x|}\times{\sf X}^{|\eta|}\to {\sf X}$.
  Then for every $a\in M^{|x|}$ and $\alpha\in({}^*\!\!X)^{|\eta|}$

  \ceq{\hfill \tau^N\big(a\,;{\rm st}(\alpha)\big)}{=}{{\rm st}\big({}^*\!\tau(a\,;\alpha)\big)}
\end{fact}
\begin{proof}
  When a $t$ is a function symbol of sort  ${\sf H}^{|x|}\to {\sf X}$, the claim holds by definition.
  When $t$ is a function symbol of sort ${\sf X}^{|\eta|}\to {\sf X}$, the claim follows from Fact~\ref{fact_terms_st}.
  Now, assume inductively that 

  \ceq{\hfill t_i^N\big(a\,;{\rm st}(\alpha)\big)}{=}{{\rm st}\big({}^*t_i(a\,;\alpha)\big)}

  holds for the terms $t_1(x\,;\eta),\dots,t_n(x\,;\eta)$ and let $t=f(t_1,\dots,t_n)$ for some function $f$ of sort ${\sf X}^n\to {\sf X}$.
  Then the claim follows immediately from the induction hypothesis and Fact~\ref{fact_terms_st}.
\end{proof}

% Symbols in ${\EuScript L}_{\sf X}$ have the interpretation they have in models.

\begin{lemma}\label{lem_st}
  With the notation as above.
  For every $\varphi(x\,;\eta)\in{\EuScript F}^{\rm p}$, \  $a\in N^{|x|}$, $\alpha\in({}^*\!\!X)^{|\eta|}$ 
  
  \ceq{\hfill\langle N,{}^*\!\!X\rangle\models\varphi(a\,;\alpha)}
  {\Rightarrow}
...
%   Let $M$ be a model and let $p(\eta)\subseteq{\EuScript F}^{\rm p}(M)$ be a type that is finitely consistent in $M$, where $\eta$ is a tuple of variables of sort ${\sf X}$.
%   Then $p(\eta)$ is realized in a model $N$ such that $M\preceq^{\rm p}N$.
% \end{corollary}

% \begin{proof}
%   By the classical Compactess Theorem, the type $p(\eta)$ is realized in some ${\EuScript L}$-structure $\langle N,{}^*\!\!X\rangle$ by
%   some $\alpha\in ({}^*\!\!X)^{|\eta|}$.
...
We say that a theory $T$ is \emph{${\EuScript L}$-consistent\/} if $\langle M,{}^*\!\!X\rangle\models T$ for some ${\EuScript L}$-structure $\langle M,{}^*\!\!X\rangle$ (this is the classical notion of concistency).
...
%   By the Classical Compactness Theorem, there is an ${\EuScript L}$-structure $\langle M,{}^*\!\!X\rangle\models T$.
%   Let $\langle M,X\rangle$ be its standard part as defined in Section~\ref{standard_part}.
...
%   Then $N\models\exists x\,\exists^X y\,p(x\,;y)$ for some model $N$ such that $M\preceq^{\rm p}N$.
% \end{corollary}
...
%   \end{itemize}
% \end{definition}
An ${\EuScript L}$-structure $N$ is \emph{positively $\lambda$-saturated\/} if it realizes all types $p(x\,;\eta)\subseteq{\EuScript F}^{\rm p}(N)$ with fewer than $\lambda$ parameters that are finitely consistent in $N$.
...
\begin{proof}
  Let $\langle {}^*\!\!M,{}^*\!\!X\rangle$ be an ${\EuScript L}$-saturated ${\EuScript L}$-elementary extension of the standard structure $\langle M,X\rangle$.
  Let $\langle {}^*\!\!M,X\rangle$ be its standard part as defined in Section~\ref{standard_part}.
  By Lemma~\ref{lem_st}, $\langle {}^*\!\!M,X\rangle$ realizes all finitely consistent positive types with $<|{}^*\!\!M|$ parameters.
\end{proof}
...
For existential quantifiers of sort ${\sf X}$ we argue similarly.% by applying Corollary~\ref{corol_Isaturation}.
%
% \noindent\llap{\textcolor{red}{\Large\danger}\kern1ex}\ignorespaces
% Consider the existential quantifiers of sort $X$
% Assume inductively
%
...
%   For a counter example, let both $M$ and $X$ are the real interval $[0,1]$.
%   The language has a function symbol ${\rm d}$ of sort $M^2\to X$.
...
A partial map is \emph{${\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}$-elementary\/} if it preserves the truth of formulas in ${\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}$.

\begin{proposition}
  Every ${\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}$-elementary map $f:{\EuScript U}\to{\EuScript U}$ of small cardinality extends to an automorphism of ${\EuScript U}$.
\end{proposition}

\begin{proof}
  By Fact~\ref{fact_max_cons_L}, the inverse of an ${\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}$-elementary map is also ${\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}$-elementary.
...
\begin{corollary}\label{corol_Lcomplete}
  Let $p(x)={\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}\mbox{-tp}(a/A)$ and $q(x)=\mbox{tp}(a/A)$. Then $p(x)\leftrightarrow q(x)$.
\end{corollary}

\begin{proof}
  Only $\rightarrow$ requires a proof.
  If $b\models p(x)$ then there is an ${\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}$-elementary map $f\supseteq{\rm id}_A$ such that $fa=b$.
  Then $f$ extends to an automorphism.
  As every automorphism is ${\EuScript L}$-elementary, $a\equiv_Ab$, and the corollary follows.
\end{proof}

The corollary says that ${\EuScript U}$ does not distinguish between ${\EuScript L}$, ${\EuScript F}^{\rm p}$, and ${\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}$ types.
...
\begin{proposition}\label{prop_LHapprox1}
  The set ${\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}$ is p-dense modulo $T$.
\end{proposition}
...
%   Let $\varphi(x)\in{\EuScript F}^{\rm p}(A)$. For every given $\tilde\varphi\perp\varphi$ there is some $\psi(x)\in{\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}(A)$ such that $\varphi(x)\rightarrow\psi(x)\rightarrow\neg\tilde\varphi(x)$.
% \end{proposition}
...
where the expression $\alpha=\beta$ is shorthand for $\langle\alpha,\beta\rangle\in\Delta$, where $\Delta$ is the diagonal of $X^2$.
...
% When two tuples of the same length $\bar a=\langle a_i:i<\lambda\rangle$ and $\bar b=\langle b_i:i<\lambda\rangle$ are such that $a_i\sim b_i$ for every $i<\lambda$ we write \emph{$\bar a\sim_{\sf X}\bar b$.}
...
We refer to this uniformity and the topology associated as the topology \emph{induced by $X$.}
Though not needed in the sequel, it is worth mentioning that the topology induced by $X$ on ${\EuScript U}^{|x|}$ coincides with the product of the topology induced by $X$ on ${\EuScript U}$.
...
% It is immediate that ${\EuScript F}^{\rm p}$-formulas constructed without using (ii) of Definition~\ref{def_LL} are ${\sf X}$-invariant.
...
We will consider two extreem cases for ${\EuScript F}$: the set of all positive/continuous formulas and the set of atomic positive/continuous formulas.
...
\begin{proof}
  ...
  ${\EuScript F}={\EuScript F}^{\rm p/c}_{\rm at}$. \ 
...
% The \emph{weight\/} of ${\EuScript L}$, which we denote by \emph{${\rm wgt}\big({\EuScript L}\big)$\/} is the minimal cardinality of a set of formulas ${\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}'$ such that for every $\varphi''>\varphi\in{\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}$ there is a $\varphi'\in{\EuScript F}^{\rm p}_{{\sf X}{\rm qf}}'$ such that $\varphi''>\varphi'>\varphi$.
% If this minimal cardinality is $\omega$, we say that ${\EuScript L}$ is sparable. 
% It is not difficult to see that when $X$ is the real unit interval and ${\EuScript L}$ contains at most countably many symbols of sort ${\sf H}^n\to{\sf X}$ then ${\EuScript L}$ is separable.
...
\ceq{\hfill\mbox{EM-p/c-tp}(b)}{=}{\bigcup_{n<\omega}\Big\{\varphi(x_{\restriction n})\in{\EuScript F}^{\rm p/c}\ \ :\ \ \varphi(b_{\restriction L_0})\textrm{ for every } L_0\subseteq L, |L_0|=n\Big\}.}
...
\begin{proof}
  ...
  \ceq{\hfill }{ }{\bigcup_{n<\omega}\Big\{\varphi(/z_{\restriction J_0})\leftrightarrow\varphi(/z_{\restriction J_1})\ \ :\ \ \varphi(x_{\restriction n})\in{\EuScript F}^{\rm p/c},\  J_i\subseteq J,\  |J_i|=n\Big\}.}
  ...
  Let $a_{\restriction J_0}\models\varphi(x_{\restriction n})$ for some $\varphi(x_{\restriction n})\in{\EuScript F}^{\rm p/c}$ and some $J_0\subseteq J$ of cardinality $n$.
...
The formulas $\varphi(x\,;z)$ and $\tilde\varphi(x\,;z)$ in Theorem~\ref{thm_stability} not required to be positive, in fact, saturation anywhere in this section. 
We prove an enhanced version of the well-kown fact that externally definable sets are internally definable when the the formula is stable.
The result is relevat to the subject of this paper when we take $\tilde\varphi\perp\varphi$.
...
The sets ${\EuScript D},\tilde{\EuScript D}\subseteq{\EuScript U}^{|z|}$ are \emph{finitely separated\/} by $\varphi(x\,;z),\tilde\varphi(x\,;z)$ if for every finite set $B\subseteq{\EuScript U}^{|z|}$ there is an $a\in{\EuScript U}^{|x|}$ such that 
...
Finally, we say that they are also \emph{honestly\/} separated if we can also guarantee that 
...
If only (iv) obtains, we say \emph{half-honestly\/} separated. 


\begin{lemma}\label{lem_sability1}
  Let $\varphi(x\,;z),\tilde\varphi(x\,;z)$ be stably separated.
  If ${\EuScript D},\tilde{\EuScript D}$ is finitely separated by $\varphi,\tilde\varphi$ then for some $m$ they are finitely and half-honestly separated by $\psi,\tilde\psi$ where 
...
\begin{lemma}\label{lem_sability2}
  Let $\varphi(x\,;z),\tilde\varphi(x\,;z)$ be stably separated.
  If ${\EuScript D},\tilde{\EuScript D}$ is finitely and half-honestly separated by $\varphi,\tilde\varphi$ then for some $m$ they are finitely and honestly separated by $\psi,\tilde\psi$ where 
...
\begin{theorem}\label{thm_stability}
  Let $\varphi(x\,;z),\tilde\varphi(x\,;z)$ be stably separated.
  If ${\EuScript D},\tilde{\EuScript D}$ is finitely separated by $\varphi,\tilde\varphi$ then for some $m,n$ they are finitely and honestly separated by $\psi,\tilde\psi$ where 
