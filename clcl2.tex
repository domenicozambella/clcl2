...
\begin{itemize}
  \item[1.] formulas using only terms of sort ${\sf H}^n\to{\sf H}$ and relations of sort ${\sf H}^n$ (up to equivalence these are nothing more than $L_{\sf H}$-formulas);
...
Their positive formulas exclude formulas as in (1) above.
Moreover they do not consider quantifiers of sort ${\sf I}$.
Including the $L_{\sf H}$-formulas into ${\EuScript L}^{\rm p}$ comes at a price e.g.\@ the failure of the Perturbation Lemma~\cite{HI}*{Proposition~5.15}.
In Section~\ref{Cauchy} we introduce the set \emph{${\EuScript L}^{\rm pc}$\/} of positive \emph{${\sf I}$-closed formulas.}
These allow in (1) only formulas that are compatible with the topology of $I$.
...
\begin{definition}\label{def_dense}
  ...
  We say dense \emph{in ${\EuScript L}^{\rm pc}$\/} if the property above is required to hold only for positive ${\sf I}$-closed formulas, defined in Section~\ref{Cauchy} below.
\end{definition}
