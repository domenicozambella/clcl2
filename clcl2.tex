...
\begin{abstract}
  ...
  Indeed, a significant part of classical model theory can be adapted, with a few caveats, to this restricted class of standard structures.
...
The notion of dense set of formulas (Definition~\ref{def_dense}) helps to reduce the size of the language when required.
...
\begin{definition}\label{def_0}
  ...
  The language ${\EuScript L}$ adds a relation symbol $R_{\varphi}(x)$ for every $\varphi(x)\in L_{\sf H}$.
  All ${\EuScript L}$-structures are assumed to model $R_{\varphi}(x)\leftrightarrow\varphi(x)$. 
  In other words, ${\EuScript L}$ performs the Morleyzation of $L_{\sf H}$.
...
We write ${\EuScript L}_x$ when when we restrict to formulas with free variables among $x$.
If $\varphi(x)\in{\EuScript L}$ contains only terms of sort ${\sf H}^n\to{\sf H}$ and relations of sort ${\sf H}^n$ we will improperly say that $\varphi(x)\in L_{\sf H}$ (which is only correct up to equivalence).
...
\begin{itemize}
  \item[1.] atomic formulas in $L_{\sf H}$ (hence, up to equivalence, all formulas of $L_{\sf H}$);
...
The positive formulas of Henson and Iovino corresponds to those formulas in ${\EuScript L}^{\rm c}$ that do not have quantifiers of sort ${\sf I}$.
...
A neater formalization is possible if one accepts that some potitive elementary extensions (to be defined below) of a normed space could contain vectors of infinite norm hence not be themselves proper normed spaces.
...
\begin{definition}\label{def_dense}
  A set of formulas ${\EuScript F}\subseteq{\EuScript L}$ is \emph{dense\/} (in ${\EuScript L}^{\rm p}$) modulo $T$, a theory, if for every positive $\varphi(x)$ and every $\varphi'>\varphi$, there is $\psi(x)\in{\EuScript F}$ such that $\varphi(x)\rightarrow\psi(x)\rightarrow\varphi'(x)$ holds in every standard structure that models $T$.
  If $T$ is empty, we say that ${\EuScript F}$ is dense modulo ${\EuScript L}$.

  We say dense \emph{in ${\EuScript L}^{\rm c}$\/} if the property above is required to hold only for positive $I$-closed formulas, defined in Section~\ref{Cauchy} below.
\end{definition}
...
\begin{example}[???]
  ...
  It is easy to see that the set of formulas build inductively from $L_{\sf H}$-formulas and $\tau(x)=s(x)$ is dense modulo ${\EuScript L}$.
\end{example}

\begin{example}
  Let ${\EuScript L}^{\rm p}_{\sf I\rm qf}$ be the set of positive formulas without quantifiers of sort ${\sf I}$.
...
We postpone the discussione of c-elementarity to Section~\ref{Cauchy}. 
As ${\EuScript L}^{\rm c}$ does not contains equality in the home sort, c-elementarity needs some extra care.

On the other hand, as p-elementary maps are in particular $L_{\sf H}$-elementary, they are injective.
...
In this section we introduce notions of morphisms based on the c
  Then $T$ is standardly consistent.
\end{theorem}
...
However, all p-models agree on the approximated truth, see Fact~\ref{fact_HImorphisms},
...
The expression $\alpha=\beta$ above is a shorthand for $\langle\alpha,\beta\rangle\in\Delta$, where $\Delta$ is the diagonal of $I^2$.
...
\begin{proof}
  ...
  Induction is easy.
  As an example, we check only the quantifier $\exists^{\scriptscriptstyle\sf H}$.
...
\begin{proposition}\label{prop_perturbation}
  ...
  and every $\varphi'>\varphi$ there are a tuple of terms $t$ and a compact neighborhood $D$ of $\Delta$ such that

    \ceq{\hfill x\sim_{\tau,D}\!y\ \wedge\ \varphi(x)}{\rightarrow}{\varphi'(y).}
\end{proposition}
...
As ${\EuScript L}^{\rm c}$ is closed under weakening and strong negation, reasoning as in Section~\ref{morphisms} we infer that for every c-model
...
\begin{proof}
  ...
  Let $a_{\tau,D}\in M$ be a realization of $a\sim_{\tau,D}x$ as $D$ ranges over the compact neighborhoods of the diagonal and $\tau(x,z)$ over the finite tuples parameter-free terms.
...
\begin{fact}
  All c-models are substructures of ${\EuScript U}$.
\end{fact}

\begin{proof}

  Let $M$ be a c-model, $a\in M^{n}$ and let $b=f(a)$ be a function symbol of sort ${\sf H}^{n}\to{\sf H}$.
  We need to prove that $b\in M$.
  It suffices to prove that $b\sim' x$ 
\end{proof}



We say that a type $q(x)$ is \emph{finitely satisfiable\/} in $A$ if every conjunction of formulas in $q(x)$ has a solution in $A$.

\begin{lemma}\label{lem_Cauchyequiv}
  The following are equivalent 
  \begin{itemize}
    \item[1.]  $p(x)\subseteq{\EuScript L}^{\rm c}(A)$ is a Cauchy type;
    \item[2.] 
    \item[3.] $a\sim'x$ is finitely satisfiable in $A$ for some/any $a\models p(x)$.
  \end{itemize}
\end{lemma}

\begin{proof}
  (1$\Rightarrow$2)
  Let $a$ be a realization of $p(x)$.
  ...
  (3$\Rightarrow$1)  
  Let $a_{\tau,D}\in M$ be a realization of $a\sim_{\tau,D}x$ as $D$ ranges over the compact neighborhoods of the diagonal and $\tau(x,z)$ over the finite tuples parameter-free terms.
  Then, by the uniqueness of the limit (up to $\sim$-equivalence), the type $p(x)$ cointaining the formulas $x\sim_{\tau,D}a_{\tau,D}$ is as required by the lemma.
\end{proof}
...
% \begin{remark}
%   Cauchy complete p-models are strong p-models.
%   I.e., $M\models\varphi$ for every true $\varphi\in{\EuScript L}^{\rm p}(M)$.
% \end{remark}

\begin{fact}
  Let $M$ be a p-model that is positively $\lambda$-saturated for some $\lambda>|{\EuScript L}|$.
...
\begin{proof}  
  ...
  By Lemma~\ref{lem_Cauchyequiv}, $p(x)\leftrightarrow a\sim x$.
  ...
  There are $|{\EuScript L}|$ many formulas in $a\sim x$.
  Then by saturation the formulas $\psi(x)$ we picked above have a common solution $b$ in $M$.
  Then $b\sim a$ and, by invariance, $b\models p(x)$.
\end{proof}
...
\begin{proposition}\label{prop_Cauchyclosure}
  ...
  Let $M$ be p-model.
  Then any Cauchy closure of $M$ is a p-model.
\end{proposition}
...
Precisely, we prove that \emph{${\EuScript L}^{\rm p}_{\sf I\rm qf}$}, the set of positive formulas without quantifiers of sort ${\sf I}$, is dense modulo $T$ (the theory of ${\EuScript U}$), see Definion~\ref{def_dense}.

We say that partial map $f:{\EuScript U}\to{\EuScript U}$ is ${\EuScript L}^{\rm p}_{\sf I\rm qf}$-elementary if it preserves the truth of formulas in ${\EuScript L}^{\rm p}_{\sf I\rm qf}$.

\begin{proposition}
  Every ${\EuScript L}^{\rm p}_{\sf I\rm qf}$-elementary map of small cardinality extends to an automorphism of ${\EuScript U}$.
\end{proposition}
...
\begin{proposition}\label{prop_LHapprox1}
  The set ${\EuScript L}^{\rm p}_{\sf I\rm qf}$ is dense modulo $T$.
\end{proposition}

\begin{proof}
  Let $\varphi(x)$ be a positive formula.
  We need to prove that for every $\varphi'>\varphi$ there is some $\psi(x)\in{\EuScript L}^{\rm p}_{\sf I\rm qf}$ such that $\varphi(x)\rightarrow\psi(x)\rightarrow\varphi'(x)$.
...
\begin{proof}
  ...
  Let $p(x)={\EuScript L}^{\rm p}_{\sf I\rm qf}\mbox{-tp}(a)$.
...
\begin{proposition}\label{prop_Sigma_approx}
  ...
  \ceq{\hfill\varphi(x)}
  {\leftrightarrow}
  {\bigvee\big\{\psi(x)\in{\EuScript L}^{\rm p}\ :\ \psi'(x)\rightarrow\varphi(x)\textrm{ for some }\psi'>\psi\big\}.}
\end{proposition}
...
The following proposition is our version of the Tarski-Vaught test (which applies when the larger p-model is sufficiently saturated).

\begin{proposition}\label{prop_Tarski_Vaught}
  Let $M$ be a \textit{subset\/} of ${\EuScript U}$.
  Let ${\EuScript F}$ be a dense set of positive formulas.
  Then the following are equivalent
  \begin{itemize}
...
We say that ${\EuScript L}$ is \emph{separable\/} modulo $T$ if there is a countable set of positive formulas that is dense modulo $T$.
...
\begin{proof}
  ...
  If $\neg\varphi(x)$ isolates $p(x)$ then any consistent formula in the disjunction on th l.h.s.\@ also isolates $p(x)$.
