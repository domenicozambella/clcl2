...
\begin{fact}\label{fact_finsat}
  Let $M$ satisfy (c) above.
  Then the following are equivalent
  \begin{itemize}
    \item[1.] $M$ is Cauchy complete (therefore it is a c-model);
...
\begin{proof}
  ...
  Let $a_{\tau,D}\in M$ be a solution of $a\sim_{\tau,D}x$ where $\tau$ ranges over the finite tuples parameter-free terms and $D$ ranges over the compact neighborhoods of the diagonal.
...
We say that ${\EuScript L}^{\rm p}(A)/{\EuScript L}^{\rm c}(A)$ is \emph{separable\/} if there is a countable subset ${\EuScript L}^{\rm p}(A)/{\EuScript L}^{\rm c}(A)$ that is p-dense/c-dense.
% In Section~\ref{separability} we will present examples of separable p-models.

\begin{proposition}\label{prop_disjunction_dense}
  Assume that ${\EuScript L}^{\rm p}(A)$ is separable modulo the theory of ${\EuScript U}$.
