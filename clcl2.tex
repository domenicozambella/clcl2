...
\begin{proof}
  ...
  \ceq{1.\hfill x\sim_\varepsilon y\wedge\varphi(x)}{\rightarrow}{\varphi'(y)}
...
Then there is a sequence $\langle\varepsilon_n:n\in\omega\rangle$ such that for every $\tau, D$ there is an $n$ such that $x\sim_{\varepsilon_n}y\ \rightarrow\ x\sim_\varepsilon y$.
We abbreviate $x\sim_{\varepsilon_n}y$ by \emph{$x\sim_ny$.}
We may assume that $x\sim_ny\ \rightarrow\ x\sim_{n+1}y$.
...
\begin{lemma}\label{lem_atomic_iso}
  Let ${\EuScript L}^{\rm c}$ be separable.
  Let $k:M\rightarrow N$ be a finite c-elementary bijection between two atomic c-models.
...
\begin{proof}
  ...
  Let $p(x,y)=\mbox{c-tp}(a,c)$, it suffices to prove that $p(x,y)$ is realized by some for some $d\in{\rm Ccl}(N)$.
  Let 
  ...
  Let $d_n\in M$ be such that $ka,d_n\models\neg\varphi_n(x,y)$.
