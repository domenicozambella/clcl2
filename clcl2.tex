...
\begin{abstract}
  ...
  In general, very little model theory is possible if we restrict to standard strutures.
  But, when $I$ is a compact topological space (and add some requirements to ${\EuScript L}$) it is possible to adapt a significant part of classical model theory.
  ...
  The main tool is the notion of approximation of a positive formulas and of its negation.
  This has been introduced by Henson and Iovino.\\[1ex]
  \noindent
  After reviewing (and elaborating on) the results of~\cite{clcl} we discuss criteria for elimination of quantifers, omitting types, $\omega$-categoricity, stability, and indiscernibles within the setting of standrd structures.
\end{abstract}
...
\begin{definition}\label{def_dense}
  ...
  We say \emph{c-dense\/} if the property above is required to hold only for continuous formulas.
\end{definition}
...
\begin{example}
  ...
  In Section~\ref{Ielimination} we will prove that this set is p-dense.
\end{example}
...
We postpone it to Section~\ref{Cauchy}.
...
Therefore,  reasoning as in Section~\ref{morphisms}, we infer that for every c-model $M$
...
\begin{proof}
  ...
  We need to prove that $fa\in M$.
  By Fact~\ref{fact_finsat}, it suffices to prove that $fa\sim' x$ is finitely satisfied in $M$.
  This is the case because $\{\exists x\ fa\sim_\tau x\}'$ holds in ${\EuScript U}$ hence, by (ac), it holds also in $M$.
\end{proof}
...
Precisely, we prove that \emph{${\EuScript L}^{\rm p}_{\sf I\rm qf}$}, the set of positive formulas without quantifiers of sort ${\sf I}$, is p-dense modulo $T$ (the theory of ${\EuScript U}$), see Definion~\ref{def_dense}.
...
\begin{proposition}\label{prop_Tarski_Vaught}
  ...
  \end{itemize}
  If $M$ is a Cauchy compelte and ${\EuScript F}$ is a c-dense set of continuous formulas then (2) and (3) above are equivalent to
  \begin{itemize}
